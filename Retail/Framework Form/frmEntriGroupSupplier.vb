...
Public Class frmEntriGroupSupplier
    ...
    End Sub
    Sub Baru()
        Dim FormEntriName As String = "EntriGroupSupplierDetil"
        Dim x As New frmSimpleEntri
        x.FormName = FormEntriName
        x.isNew = True
        x.IDParent = NoID
        If x.ShowDialog(Me) = Windows.Forms.DialogResult.OK Then
            EksekusiSQL("UPDATE MGroupSupplierD SET IDGroupSupplier=" & NoID & " WHERE MGroupSupplierD.NoID=" & x.NoID)
            RefreshDetil()
    ...
    Sub Hapus()
        ...
        Try
            ...
            EksekusiSQL("DELETE MGroupSupplierD WHERE NoID=" & NoID.ToString)
            RefreshDetil()
        Catch ex As Exception
    ...
    Sub Edit()
        ...
        Catch ex As Exception
            XtraMessageBox.Show("Untuk mengedit pilih data alamat yang akan diedit terlebih dahulu lalu tekan tombol edit", NamaAplikasi, MessageBoxButtons.OK, MessageBoxIcon.Error)
        Finally
    ...
    Sub RefreshDetil()
        ...
        ocmd2.CommandText = "SELECT MGroupSupplierD.*, MAlamat.Nama AS NamaSupplier, MAlamat.Kode AS KodeSupplier, MAlamat.IsPKP AS PKP, MAlamat.JatuhTempoSupplier AS JatuhTempo, MAlamat.Dept FROM MGroupSupplierD " & _
                            "LEFT JOIN MAlamat ON MAlamat.NoID=MGroupSupplierD.IDAlamat " & _
                            "WHERE MGroupSupplierD.IDGroupSupplier=" & NoID
        oda2 = New SqlDataAdapter(ocmd2)
