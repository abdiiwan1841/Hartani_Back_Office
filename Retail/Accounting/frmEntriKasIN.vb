...
Public Class frmEntriKasIN
    ...
    'Public FormPemanggil As frmDaftarKasIN
    ...
    Private Sub SimpleButton9_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles SimpleButton9.Click
        ...
        If GridView1.RowCount >= 1 Then
            If Simpan() Then
                If Not NullToBool(EksekusiSQlSkalarNew("SELECT IsPosted FROM MKasIN WHERE NoID=" & NoID)) Then
                    frmDaftarKasIN.PostingLainlain(NoID, NullToStr(EksekusiSQlSkalarNew("SELECT Keterangan FROM MKasIN WHERE NoID=" & NoID)))
                    EksekusiSQL("Update MKasIN set IsPosted=1 where ID=" & NoID)
                End If
            ...
            If isValidasi() Then
                If pStatus = mdlAccPublik.ptipe.Baru Then
                    ...
                    SQL = "Insert Into MKasIN (IDGudang,ID,Tanggal,Kode,Keterangan,Jumlah," & _
                          "IsPosted,IDAkunKas,IDKas,KodeReff,IDWIlayah,IDAlamat,NoKwitansi) Values(" & _
                          IDGudang & "," & NoID & ",'" & Format(Tgl.DateTime, "yyyy/MM/dd") & "','" & FixApostropi(txtKode.Text) & "','" & FixApostropi(txtCatatan.Text) & "'," & _
                          FixKoma(txtJumlah.EditValue) & ",0," & IDAkun & "," & IDKasBank & ",'" & FixApostropi(txtKodeReff.Text) & "'," & DefIDWilayah & "," & NullToLong(txtKodeSupplier.EditValue) & ",'" & FixApostropi(txtNoKwitansi.Text) & "')"
    ...
    Private Sub SetCtlMe()
        ...
        Try
            ds = ExecuteDataset("MAlamat", "SELECT MAlamat.NoID, MAlamat.Kode, MAlamat.Nama, MAlamat.Alamat FROM MAlamat WHERE MAlamat.IsActive=1")
            txtKodeSupplier.Properties.DataSource = ds.Tables("MAlamat")
            txtKodeSupplier.Properties.ValueMember = "NoID"
            txtKodeSupplier.Properties.DisplayMember = "Kode"
            If txtKas.Text = "" Then
        ...
        ElseIf pStatus = mdlAccPublik.ptipe.Edit Then
            ...
            Try
                ...
                Else
                    ...
                    txtNoKwitansi.Text = NullToStr(oDS.Tables("MTable").Rows(0).Item("NoKwitansi"))
    ...
    Private Sub Tgl_EditValueChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Tgl.EditValueChanged
        Try
            If pStatus = mdlAccPublik.ptipe.Baru Then
                txtKode.Text = clsKode.MintaKodeBaru("KM", "MKasIn", Tgl.DateTime, DefIDWilayah, 5)
            End If
        Catch ex As Exception
