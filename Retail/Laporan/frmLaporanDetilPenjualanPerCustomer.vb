...
Public Class frmLaporanDetilPenjualanPerCustomer
    Public FormName As String = "LaporanDetilPenjualanPerCustomer"
    ...
    Private Sub ctrlDaftar_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Try
            ...
            RefreshDataKontak()
            txtKodeCustomer.EditValue = IDCustomer
            RefreshData()
            RestoreLayout()
    ...
    Sub RefreshData()
        ...
        Try
            ...
            If RadioButton2.Checked Then
                strsql = "Select MAlamat.Kode as KodeMember,MAlamat.Nama as Customer,MAlamat.Alamat,Malamat.Telpon,MBarang.Kode as KodeBarang,MBarang.Nama as Barang,Sales.* From Malamat "
                strsql = strsql & " inner Join "
                strsql = strsql & " (Select MJual.IDCustomer,Count(distinct MJual.NoID) as Kunjungan,MJualD.IDBarang,Sum(MJualD.Qty) as Qty,Sum(MJualD.Jumlah) as SubTotal "
                strsql = strsql & " from MJualD Inner Join MJual On MJualD.IDJual =MJual.NoID  "
                ...
                strsql = strsql & " group by MJual.IDCustomer,MJualD.IDBarang   ) Sales "
                strsql = strsql & " on Sales.IDCustomer=MAlamat.NoID "
                strsql = strsql & " inner join MBarang on Sales.IDBarang=MBarang.NoID "
                strsql = strsql & IIf(txtKodeCustomer.Enabled, "where Malamat.NoID= " & NullTolInt(txtKodeCustomer.EditValue), " ")
                strsql = strsql & " order by Sales.SubTotal Desc "
            ElseIf RadioButton1.Checked Then
                ...
                strsql = strsql & " inner join MBarang on Sales.IDBarang=MBarang.NoID "
                strsql = strsql & IIf(txtKodeCustomer.Enabled, "where Malamat.NoID= " & NullTolInt(txtKodeCustomer.EditValue), " ")
                strsql = strsql & " order by Sales.Qty Desc "
            End If
    ...
    Private Sub RefreshDataKontak()
        ...
        Dim strsql As String
        Try
            strsql = "SELECT NoID, Kode, Nama FROM MAlamat WHERE IsCustomer=1"
            ds = ExecuteDataset("master", strsql)
    ...
    Private Sub LabelControl4_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles LabelControl4.Click
        txtKodeCustomer.Enabled = Not txtKodeCustomer.Enabled
    End Sub
End Class