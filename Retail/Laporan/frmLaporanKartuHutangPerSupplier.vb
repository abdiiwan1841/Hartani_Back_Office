...
Public Class frmLaporanKartuHutangPerSupplier
        ...
        Select Case e.Button.Index
            Case 0
                ...
                If x.ShowDialog(Me) = Windows.Forms.DialogResult.OK Then
                    IDAlamat = x.NoID
                    txtAlamat.Text = x.Kode & " - " & x.Nama & " - " & x.Kontak
                End If
                x.Dispose()
            Case 1
                IDAlamat = -1
    ...
    Sub RefreshItem()
        ...
        Try
            ...
            strsql = "SELECT 18 AS IDJenisTransaksi, DATEADD(dd, 0, DATEDIFF(dd, 0, MSaldoAwalHutangPiutang.Tanggal)) AS Tanggal, MAlamat.Kode AS KodeSupplier, MAlamat.Nama AS Supplier, MSaldoAwalHutangPiutang.Kode AS NoNota, MSaldoAwalHutangPiutang.SaldoAwal AS Masuk, 0 AS Keluar, 'Saldo Awal' AS Keterangan" & vbCrLf & _
                     ...
                     " WHERE MSaldoAwalHutangPiutang.IsPosted=1 AND MAlamat.NoID=" & IDAlamat & " AND MSaldoAwalHutangPiutang.Tanggal>='" & TglDari.DateTime.ToString("yyyy-MM-dd") & "' AND MSaldoAwalHutangPiutang.Tanggal<'" & TglSampai.DateTime.AddDays(1).ToString("yyyy-MM-dd") & "'" & vbCrLf & _
                     " UNION ALL" & vbCrLf & _
                     " SELECT 2, DATEADD(dd, 0, DATEDIFF(dd, 0, MBeli.Tanggal)) AS Tanggal, MAlamat.Kode AS KodeSupplier, MAlamat.Nama AS Supplier, MBeli.Kode AS NoNota, MBeli.Total AS Masuk, 0 AS Keluar, 'Pembelian' AS Keterangan" & vbCrLf & _
                     " FROM MBeli INNER JOIN MAlamat ON MAlamat.NoID=MBeli.IDSupplier" & vbCrLf & _
                     " WHERE MBeli.IsPosted=1 AND MAlamat.NoID=" & IDAlamat & " AND MBeli.Tanggal>='2012-12-01' AND MBeli.Tanggal>='" & TglDari.DateTime.ToString("yyyy-MM-dd") & "' AND MBeli.Tanggal<'" & TglSampai.DateTime.AddDays(1).ToString("yyyy-MM-dd") & "'" & vbCrLf & _
                     " UNION ALL" & vbCrLf & _
                     " SELECT 3, DATEADD(dd, 0, DATEDIFF(dd, 0, MBayarHutang.TglKembali)) AS Tanggal, MAlamat.Kode AS KodeSupplier, MAlamat.Nama AS Supplier, MReturBeli.Kode AS NoNota, 0, MReturBeli.Total AS Keluar, 'Retur Pembelian' AS Keterangan" & vbCrLf & _
                     ...
                     " WHERE MReturBeli.IsPosted=1 AND MAlamat.NoID=" & IDAlamat & " AND MBayarHutang.TglKembali>='2012-12-01' AND MBayarHutang.TglKembali>='" & TglDari.DateTime.ToString("yyyy-MM-dd") & "' AND MBayarHutang.TglKembali<'" & TglSampai.DateTime.AddDays(1).ToString("yyyy-MM-dd") & "'" & vbCrLf & _
                     " UNION ALL " & vbCrLf & _
                     " SELECT 17, DATEADD(dd, 0, DATEDIFF(dd, 0, MBayarHutang.TglKembali)) AS Tanggal, MAlamat.Kode AS KodeSupplier, MAlamat.Nama AS Supplier, MBayarHutang.Kode AS NoNota, 0, MBayarHutang.Total AS Masuk, 'Pembayaran Hutang' AS Keterangan" & vbCrLf & _
                     " FROM MBayarHutang INNER JOIN MAlamat ON MAlamat.NoID=MBayarHutang.IDAlamat" & vbCrLf & _
                     " WHERE MBayarHutang.IsPosted=1 AND MAlamat.NoID=" & IDAlamat & " AND MBayarHutang.TglKembali>='2012-12-01' AND MBayarHutang.TglKembali>='" & TglDari.DateTime.ToString("yyyy-MM-dd") & "' AND MBayarHutang.TglKembali<'" & TglSampai.DateTime.AddDays(1).ToString("yyyy-MM-dd") & "'" & vbCrLf & _
                     " UNION ALL" & vbCrLf & _
                     " SELECT 17, DATEADD(dd, 0, DATEDIFF(dd, 0, MBayarHutang.TglKembali)) AS Tanggal, MAlamat.Kode AS KodeSupplier, MAlamat.Nama AS Supplier, MBayarHutang.Kode AS NoNota, SUM(MBayarHutangDDebet.Potong) AS Masuk, 0, 'DN Nota' AS Keterangan" & vbCrLf & _
                     ...
                     " UNION ALL" & vbCrLf & _
                     " SELECT 17, DATEADD(dd, 0, DATEDIFF(dd, 0, MBayarHutang.TglKembali)) AS Tanggal, MAlamat.Kode AS KodeSupplier, MAlamat.Nama AS Supplier, MBayarHutang.Kode AS NoNota, 0, ISNULL(SUM(MBayarHutangDKredit.Potong),0) AS Keluar, 'CN Nota' AS Keterangan" & vbCrLf & _
                     ...
                     " UNION ALL" & vbCrLf & _
                     " SELECT 17, DATEADD(dd, 0, DATEDIFF(dd, 0, MBayarHutang.TglKembali)) AS Tanggal, MAlamat.Kode AS KodeSupplier, MAlamat.Nama AS Supplier, MBayarHutang.Kode AS NoNota, 0, IsNull(MBayarHutang.Materai,0) AS Keluar, 'Materai Pembayaran' AS Keterangan" & vbCrLf & _
                     " FROM MBayarHutang INNER JOIN MAlamat ON MAlamat.NoID=MBayarHutang.IDAlamat" & vbCrLf & _
                     " WHERE MBayarHutang.IsPosted=1 AND MAlamat.NoID=" & IDAlamat & " AND MBayarHutang.TglKembali>='2012-12-01' AND MBayarHutang.TglKembali>='" & TglDari.DateTime.ToString("yyyy-MM-dd") & "' AND MBayarHutang.TglKembali<'" & TglSampai.DateTime.AddDays(1).ToString("yyyy-MM-dd") & "' AND IsNull(MBayarHutang.Materai,0)<>0" & vbCrLf & _
                     " UNION ALL" & vbCrLf & _
                     " SELECT 17, DATEADD(dd, 0, DATEDIFF(dd, 0, MBayarHutang.TglKembali)) AS Tanggal, MAlamat.Kode AS KodeSupplier, MAlamat.Nama AS Supplier, MBayarHutang.Kode AS NoNota, 0, IsNull(MBayarHutang.JumlahKwitansi,0) AS Keluar, 'Kwitansi Pembayaran' AS Keterangan" & vbCrLf & _
                     " FROM MBayarHutang INNER JOIN MAlamat ON MAlamat.NoID=MBayarHutang.IDAlamat" & vbCrLf & _
                     " WHERE MBayarHutang.IsPosted=1 AND MAlamat.NoID=" & IDAlamat & " AND MBayarHutang.TglKembali>='2012-12-01' AND MBayarHutang.TglKembali>='" & TglDari.DateTime.ToString("yyyy-MM-dd") & "' AND MBayarHutang.TglKembali<'" & TglSampai.DateTime.AddDays(1).ToString("yyyy-MM-dd") & "' AND IsNull(MBayarHutang.JumlahKwitansi,0)<>0" & vbCrLf
    ...
    Private Sub HitungSaldo()
        ...
        Try
            SQL = "SELECT MSaldoAwalHutangPiutang.SaldoAwal-0 AS Total" & vbCrLf & _
                 " FROM MSaldoAwalHutangPiutang INNER JOIN MAlamat ON MAlamat.NoID=MSaldoAwalHutangPiutang.IDAlamat" & vbCrLf & _
                 " WHERE MSaldoAwalHutangPiutang.IsPosted=1 AND MAlamat.NoID=" & IDAlamat & " AND MSaldoAwalHutangPiutang.Tanggal<'" & TglDari.DateTime.ToString("yyyy-MM-dd") & "'" & vbCrLf & _
                 " UNION ALL" & vbCrLf & _
                 " SELECT MBeli.Total-0 AS Total" & vbCrLf & _
                 " FROM MBeli INNER JOIN MAlamat ON MAlamat.NoID=MBeli.IDSupplier" & vbCrLf & _
                 " WHERE MBeli.IsPosted=1 AND MAlamat.NoID=" & IDAlamat & " AND MBeli.Tanggal>='2012-12-01' AND MBeli.Tanggal<'" & TglDari.DateTime.ToString("yyyy-MM-dd") & "'" & vbCrLf & _
                 " UNION ALL" & vbCrLf & _
                 " SELECT 0-MReturBeli.Total" & vbCrLf & _
                 " FROM MReturBeli INNER JOIN MAlamat ON MAlamat.NoID=MReturBeli.IDSupplier" & vbCrLf & _
                 " INNER JOIN (MBayarHutang INNER JOIN MBayarHutangDRetur ON MBayarHutang.NoID=MBayarHutangDRetur.IDBayarHutang) ON MBayarHutangDRetur.IDReturBeli=MReturBeli.NoID" & vbCrLf & _
                 " WHERE MReturBeli.IsPosted=1 AND MAlamat.NoID=" & IDAlamat & " AND MBayarHutang.TglKembali>='2012-12-01' AND MBayarHutang.TglKembali<'" & TglDari.DateTime.ToString("yyyy-MM-dd") & "'" & vbCrLf & _
                 " UNION ALL " & vbCrLf & _
                 " SELECT 0-MBayarHutang.Total AS Masuk" & vbCrLf & _
                 " FROM MBayarHutang INNER JOIN MAlamat ON MAlamat.NoID=MBayarHutang.IDAlamat" & vbCrLf & _
                 " WHERE MBayarHutang.IsPosted=1 AND MAlamat.NoID=" & IDAlamat & " AND MBayarHutang.TglKembali>='2012-12-01' AND MBayarHutang.TglKembali<'" & TglDari.DateTime.ToString("yyyy-MM-dd") & "'" & vbCrLf & _
                 " UNION ALL" & vbCrLf & _
                 ...
                 " FROM MBayarHutang INNER JOIN MAlamat ON MAlamat.NoID=MBayarHutang.IDAlamat" & vbCrLf & _
                 " WHERE MBayarHutang.IsPosted=1 AND MAlamat.NoID=" & IDAlamat & " AND MBayarHutang.TglKembali>='2012-12-01' AND MBayarHutang.TglKembali<'" & TglDari.DateTime.ToString("yyyy-MM-dd") & "' AND IsNull(MBayarHutang.Materai,0)<>0" & vbCrLf & _
                 " UNION ALL " & vbCrLf & _
                 " SELECT 0-IsNull(MBayarHutang.JumlahKwitansi,0) AS Keluar" & vbCrLf & _
                 " FROM MBayarHutang INNER JOIN MAlamat ON MAlamat.NoID=MBayarHutang.IDAlamat" & vbCrLf & _
                 " WHERE MBayarHutang.IsPosted=1 AND MAlamat.NoID=" & IDAlamat & " AND MBayarHutang.TglKembali>='2012-12-01' AND MBayarHutang.TglKembali<'" & TglDari.DateTime.ToString("yyyy-MM-dd") & "' AND IsNull(MBayarHutang.JumlahKwitansi,0)<>0"
