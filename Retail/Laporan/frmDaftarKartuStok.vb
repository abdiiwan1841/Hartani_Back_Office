...
Public Class frmDaftarKartuStok
    ...
    Private Sub RefreshLookUpBarang()
        ...
        Dim SQL As String = ""
        Try
            SQL = "SELECT MBarang.NoID,MBarang.Kode,MBarang.Nama,MSatuan.Nama Satuan " & _
            "from MBarang Left Join MSatuan On MBarang.IDSatuan= MSatuan.NoID " & _
    ...
    Sub RefreshData()
        ...
        Try
            ...
            If TglDari.Enabled Then
                If txtGudang.Text <> "" Then
                    SQL = "SELECT SUM((MKartuStok.QtyMasuk-MKartuStok.QtyKeluar)*MKartuStok.Konversi) AS Saldo" & vbCrLf & _
                          ...
                          " INNER JOIN (MBarang INNER JOIN MBarangD ON MBarang.NoID=MBarangD.IDBarang) ON MBarang.NoID=MKartuStok.IDBarang AND MBarangD.NoID=MKartuStok.IDBarangD " & vbCrLf & _
                          " INNER JOIN MGudang ON MGudang.NoID=MKartuStok.IDGudang " & vbCrLf & _
                          " WHERE MKartuStok.Tanggal<'" & TglDari.DateTime.ToString("yyyy-MM-dd") & "' AND MGudang.NoID=" & NullToLong(txtGudang.EditValue) & " AND MBarang.NoID=" & NullToLong(txtBarang.EditValue)
                Else
                    SQL = "SELECT SUM((MKartuStok.QtyMasuk-MKartuStok.QtyKeluar)*MKartuStok.Konversi) AS Saldo" & vbCrLf & _
                          " FROM MKartuStok " & vbCrLf & _
                          " INNER JOIN (MBarang INNER JOIN MBarangD ON MBarang.NoID=MBarangD.IDBarang) ON MBarang.NoID=MKartuStok.IDBarang AND MBarangD.NoID=MKartuStok.IDBarangD " & vbCrLf & _
                          " INNER JOIN MGudang ON MGudang.NoID=MKartuStok.IDGudang " & vbCrLf & _
                          " WHERE MKartuStok.Tanggal<'" & TglDari.DateTime.ToString("yyyy-MM-dd") & "' AND MBarang.NoID=" & NullToLong(txtBarang.EditValue)
                End If
            ...
            SQL &= " MKategori.Nama AS Kategori, MKartuStok.HargaBeli/MKartuStok.Konversi AS HargaPcs,MBarang.IsActive, MJenisTransaksi.Nama AS Transaksi,MAlamat.Nama as Supplier " & vbCrLf & _
            ...
            SQL &= " FROM MKartuStok INNER JOIN ((MBarang INNER JOIN MBarangD ON MBarang.NoID=MBarangD.IDBarang) LEFT OUTER JOIN" & vbCrLf
            SQL &= " MKategori ON MBarang.IDKategori = MKategori.NoID LEFT OUTER JOIN" & vbCrLf
            SQL &= " MJenisBarang ON MBarang.IDJenis = mjenisbarang.NoID) ON MBarang.NoID=MKartuStok.IDBarang AND MBarangD.NoID=MKartuStok.IDBarangD " & vbCrLf
    ...
    Sub PrintPreview()
        ...
        Dim namafile As String = Application.StartupPath & "\Report\DaftarKartuStok.rpt"
        Try
            RefreshData()
            If EditReport Then
                mdlCetakCR.ViewReport(Me.MdiParent, mdlCetakCR.action_.Edit, namafile, "Daftar Kartu Stok", , , "SaldoAwal=" & txtSaldoAwal.EditValue & "&IDBarangD=" & NullToLong(txtBarang.EditValue) & "&TglDari=cdate(" & TglDari.DateTime.ToString("yyyy,MM,dd") & ")&TglSampai=cdate(" & TglSampai.DateTime.ToString("yyyy,MM,dd") & ")")
            Else
                mdlCetakCR.ViewReport(Me.MdiParent, mdlCetakCR.action_.Preview, namafile, "Daftar Kartu Stok", , , "SaldoAwal=" & txtSaldoAwal.EditValue & "&IDBarangD=" & NullToLong(txtBarang.EditValue) & "&TglDari=cdate(" & TglDari.DateTime.ToString("yyyy,MM,dd") & ")&TglSampai=cdate(" & TglSampai.DateTime.ToString("yyyy,MM,dd") & ")")
            End If
        Catch ex As Exception
    ...
    Private Sub TextEdit1_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles TextEdit1.KeyDown
        ...
        ElseIf e.KeyCode = Keys.Enter Then
            txtBarang.EditValue = NullToLong(EksekusiSQlSkalarNew("Select MBarangD.IDBarang from MBarangD where barcode='" & FixApostropi(TextEdit1.Text) & "'"))
        End If
    End Sub
End Class