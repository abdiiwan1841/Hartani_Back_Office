...
Public Class frmLaporanPembelianvsPenjualanOld
        ...
        Try
            ...
            ElseIf TableMaster.ToUpper = "MBARANG".ToUpper Then
                ...
                ElseIf TglDari.Enabled Then
                    strsql = "SELECT NoID,KodeBarang,Barcode,NamaBarang,QtyMinimum,IDBarang,Wilayah "
                    For i As Integer = 0 To dsGudang.Tables(0).Rows.Count - 1
                    ...
                    strsql = strsql & ", SUM(TotalQty) AS TotalQty FROM (" & _
                             "SELECT MBarang.NoID, MBarang.Kode AS KodeBarang,MBarang.Barcode, MBarang.Nama AS NamaBarang,MBarang.StockMin as QtyMinimum,p.*, (SELECT SUM((MKartuStok.QtyMasuk*MKartuStok.Konversi)-(MKartuStok.QtyKeluar*MKartuStok.Konversi)) FROM MKartuStok LEFT JOIN MGudang ON MGudang.NoID=MKartuStok.IDGudang WHERE MGudang.NoID IN (SELECT MUserDAksesGudang.IDGudang FROM MUserDAksesGudang WHERE MUserDAksesGudang.IsActive=1 AND MUserDAksesGudang.IDUser=" & IDUserAktif & ") AND MBarang.NoID=MKartuStok.IDBarang AND (MKartuStok.Tanggal>='" & Format(TglDari.DateTime, "yyyy/MM/dd") & "' and MKartuStok.Tanggal<'" & Format(DateAdd(DateInterval.Day, 1, TglSampai.DateTime), "yyyy/MM/dd") & "') ) AS TotalQty" & vbCrLf & _
                             ...
                             " GROUP BY NoID,KodeBarang,Barcode,NamaBarang,QtyMinimum,IDBarang,Wilayah"
                Else
                    strsql = "SELECT NoID,KodeBarang,Barcode,NamaBarang,QtyMinimum,IDBarang,Wilayah "
                    For i As Integer = 0 To dsGudang.Tables(0).Rows.Count - 1
                        strsql = strsql & ", SUM(" & NullToStr(dsGudang.Tables(0).Rows(i).Item("Kode")) & ") AS " & NullToStr(dsGudang.Tables(0).Rows(i).Item("Kode"))
    ...
    Private Sub txtKodeCustomer_EditValueChanged(ByVal sender As System.Object, ByVal e As System.EventArgs)
