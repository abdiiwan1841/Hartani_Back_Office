...
Public Class frmLaporanLabaKotorPerKategori
    Public FormName As String = "LaporanLabaPerKategori"
    ...
    Sub RefreshData()
        ...
        Try
            ...
            '         " From " & _
            '         " MJualD Inner Join MJual On MJualD.IDJual=MJual.NoID INNER JOIN MBarang ON MBarang.NoID=MJualD.IDBarang INNER JOIN MKategori ON MKategori.NoID=MBArang.IDKategori where MJual.Tanggal>='" & Format(TglDari.DateTime, "yyyy/MM/dd") & "' AND MJual.Tanggal<'" & Format(TglSampai.DateTime.AddDays(1), "yyyy/MM/dd") & "' group by MKategori.NoID, MKategori.DefProvitMargin) Omset" & _
            '         " On MKategori.NoID=Omset.IDKategori WHERE MKategori.IsActive=1 "
            ''If txtKategori.Enabled And txtKategori.Text.Trim <> "" Then
            ''    strsql = strsql & " where MBarang.IDKategori=" & NullToLong(txtKategori.EditValue)
            ''End If

            'Dirubah Tgl 10-07-2014 By Request Bp. Vincent Rms : 100-(HargaBeli/HargaJual*100%)
            strsql = "SELECT MKategori.NoID, MKategori.Nama, MKategori.Kode, Omset.* From MKategori Left Join " & _
                     " (Select MKategori.NoID AS IDKategori, Sum(MJualD.Qty*MJualD.Konversi) as QtyPenjualan, Sum(MJualD.Jumlah-IsNull(MJualD.JumlahDiscNotaRp,0)) AS Penjualan, " & vbCrLf & _
                     ...
                     " CASE WHEN IsNull(MKategori.DefProvitMargin,0)=0 THEN Sum(ISNULL(MJualD.Qty,0) * ISNULL(MJualD.HargaPokok,0)) ELSE Sum(MJualD.Jumlah-IsNull(MJualD.JumlahDiscNotaRp,0)) * (1-(MKategori.DefProvitMargin / 100)) END as JumlahHPP, " & vbCrLf & _
                     " CASE WHEN IsNull(Mkategori.DefProvitMargin,0)=0 THEN (case when Sum(MJualD.Jumlah-IsNull(MJualD.JumlahDiscNotaRp,0))<>0 AND (Sum(ISNULL(MJualD.Qty,0) * ISNULL(MJualD.HargaPokok,0)))<>0 then 100-(Sum(ISNULL(MJualD.Qty,0) * ISNULL(MJualD.HargaPokok,0))/Sum(MJualD.Jumlah-IsNull(MJualD.JumlahDiscNotaRp,0))*100) else 0.0 end) ELSE Mkategori.DefProvitMargin END AS [MarginLaba (%)], " & vbCrLf & _
                     " Count(distinct MJualD.IDJual) as Pembeli " & _
                     " From " & _
                     " MJualD Inner Join MJual On MJualD.IDJual=MJual.NoID INNER JOIN MBarang ON MBarang.NoID=MJualD.IDBarang INNER JOIN MKategori ON MKategori.NoID=MBArang.IDKategori where MJual.Tanggal>='" & Format(TglDari.DateTime, "yyyy/MM/dd") & "' AND MJual.Tanggal<'" & Format(TglSampai.DateTime.AddDays(1), "yyyy/MM/dd") & "' group by MKategori.NoID, MKategori.DefProvitMargin) Omset" & _
                     " On MKategori.NoID=Omset.IDKategori WHERE MKategori.IsActive=1 "
            If txtKategori.Enabled And txtKategori.Text.Trim <> "" Then
                strsql = strsql & " AND MKategori.NoID<>" & NullToLong(txtKategori.EditValue)
            End If
            ...
            ElseIf RadioButton4.Checked Then
                strsql = strsql & " order by MKategori.Kode "
            ElseIf RadioButton5.Checked Then
                strsql = strsql & " order by MKategori.Nama "
    ...
    Sub PrintPreview()
        ...
        Try
            ...
            clsCetakReportDevExpress.ViewXtraReport(Me.MdiParent, IIf(EditReport, mdlCetakCR.action_.Edit, mdlCetakCR.action_.Preview), Application.StartupPath & "\Report\LaporanLabaPerKategori.repx", "Laporan Laba Per Kategori", "LaporanLabaPerKategori.repx", ds, , "TglDari=Datetime=#" & TglDari.DateTime.ToString("yyyy/MM/dd") & "#|TglSampai=Datetime=#" & TglSampai.DateTime.ToString("yyyy/MM/dd") & "#|ProvitMargin=Double=" & Provit & "|NonKategori=String='" & FixApostropi(txtKategori.Text) & "'")
        Catch ex As Exception
            XtraMessageBox.Show("Info Kesalahan : " & ex.Message, NamaAplikasi, MessageBoxButtons.OK, MessageBoxIcon.Information)
        End Try
        'CetakMRPTJual(IIf(EditReport, action_.Edit, action_.Preview))
        'clsCetakReportDevExpress.ViewXtraReport(Me.MdiParent, IIf(EditReport, mdlCetakCR.action_.Edit, mdlCetakCR.action_.Preview), Application.StartupPath & "\Report\LaporanLabaPerKategori.repx", "Laporan Laba Per Kategori", "LaporanLabaPerKategori.repx", ds, , "TglDari=Datetime=#" & TglDari.DateTime.ToString("yyyy/MM/dd") & "#|TglSampai=Datetime=#" & TglSampai.DateTime.ToString("yyyy/MM/dd") & "#")
    End Sub
