...
Public Class frmLaporanSaldoStok
    Public FormName As String = "LaporanSaldoStok"
    ...
    Sub RefreshData()
        ...
        Try
            ...
            If NamaGudang = "" Then
                XtraMessageBox.Show("Anda Tidak Punya Akses.", NamaAplikasi, MessageBoxButtons.OK, MessageBoxIcon.Information)
                Exit Try
            ElseIf TglSampai.Enabled Then
                'Laporan terpakai
                strsql = "SELECT MBarang.NoID, MBarang.IsNewItem AS ProdukBaru, MBarang.HargaBeliPcs AS HBeli, MBarang.HargaJual AS HJual, MBarang.IsActive AS BarangAktif, MKategori.Kode AS KodeKategori, MKategori.Nama AS Kategori, MSup1.Kode + ' - ' + MSup1.Nama AS Supplier1, MSup5.Kode + ' - ' + MSup5.Nama AS Supplier5, MBarang.Kode, MBarang.Barcode, MBarang.Nama, MBarang.StockMin AS MinimalStock "
                For i As Integer = 0 To dsGudang.Tables(0).Rows.Count - 1
            ...
            'Di Detilkan terus di group by idbarang
            SQL = "SELECT IsNull((SELECT COUNT(MBarangD.NoID) FROM MBarangD WHERE MBarangD.IDBarang=MStok.NoID),0) AS JumlahBarcodeAktif, MStok.BarangAktif, MStok.ProdukBaru, MStok.KodeKategori, MStok.Kategori, MStok.Supplier1, MStok.Supplier5, MStok.Kode, MStok.Barcode, MStok.Nama, MStok.MinimalStock, SUM(MStok.TotalQty) AS TotalQty, SUM(MStok.TotalQty)*MStok.HBeli AS Nilai, SUM(MStok.TotalQty)*MStok.HJual AS NilaiJual, MStok.HBeli, MStok.HJual " & vbCrLf
            For i As Integer = 0 To dsGudang.Tables(0).Rows.Count - 1
                SQL &= ", SUM(MStok." & NullToStr(dsGudang.Tables(0).Rows(i).Item("Kode")) & ") AS " & NullToStr(dsGudang.Tables(0).Rows(i).Item("Kode"))
            Next
            SQL &= " FROM (" & strsql & ") AS MStok " & vbCrLf & _
                   " GROUP BY MStok.NoID, MStok.BarangAktif, MStok.ProdukBaru, MStok.KodeKategori, MStok.Kategori, MStok.Supplier1, MStok.Supplier5, MStok.Kode, MStok.Barcode, MStok.Nama, MStok.MinimalStock, MStok.HBeli, MStok.HJual "
            If RadioButton1.Checked Then ' 
                ...
                '    strsql = strsql & " and "
                'End If
                SQL &= " HAVING SUM(MStok.TotalQty)<0"
                isperluwhere = False
            ...
            ElseIf RadioButton3.Checked Then
                ...
                '    strsql = strsql & " and "
                'End If
                SQL &= " HAVING SUM(MStok.TotalQty)<MStok.MinimalStock"
                isperluwhere = False
