...
Public Class frmEntriRequestMutasiWilayah
    Dim SQL As String = ""
    Public Enum pStatus
        Baru = 0
        Edit = 1
    ...
    Private Sub RefreshDataKontak()
        ...
        Try
            ...
            SQL = "SELECT NoID, Kode, Nama FROM MWilayah WHERE IsActive=1 AND NoID<>" & NullTolong(txtWilayah.EditValue)
            ds = ExecuteDataset("master", SQL)
    ...
    Private Sub frmEntriPO_FormClosing(ByVal sender As Object, ByVal e As System.Windows.Forms.FormClosingEventArgs) Handles Me.FormClosing
        If IsTempInsertBaru Then
            If XtraMessageBox.Show("Yakin ingin keluar?" & vbCrLf & "Ya, akan menghapus transaksi dan detil item.", NamaAplikasi, MessageBoxButtons.YesNo, MessageBoxIcon.Information, MessageBoxDefaultButton.Button2) = Windows.Forms.DialogResult.Yes Then
                EksekusiSQL("DELETE FROM MRequestMutasiWilayah WHERE NoID=" & NoID)
                EksekusiSQL("DELETE FROM MRequestMutasiWilayahD WHERE IDHeader=" & NoID)
            Else
                e.Cancel = True
            End If
        End If
    ...
    Private Sub RefreshData()
        ...
        Try
            ...
            If pTipe = pStatus.Baru Then
                IsiDefault()
            Else
                SQL = "SELECT MRequestMutasiWilayah.*, MUserEntri.Nama AS UserEntri, MUserEdit.Nama AS UserEdit, MUserPosting.Nama AS UserPosting, MAlamat.Alamat AS Alamat, MAlamat.Nama AS NamaAlamat, MAlamat.Kode as KodeAlamat "
                SQL &= " FROM MRequestMutasiWilayah LEFT JOIN MAlamat ON MAlamat.NoID=MRequestMutasiWilayah.IDPegawaiRequest "
                SQL &= " LEFT JOIN MUser MUserEntri ON MUserEntri.NoID=MRequestMutasiWilayah.IDUserEntry "
                SQL &= " LEFT JOIN MUser MUserEdit ON MUserEdit.NoID=MRequestMutasiWilayah.IDUserEdit "
                SQL &= " LEFT JOIN MUser MUserPosting ON MUserPosting.NoID=MRequestMutasiWilayah.IDUserPosting "
                SQL &= " WHERE MRequestMutasiWilayah.NoID = " & NoID
                DS = ExecuteDataset("Tabel", SQL)
                If DS.Tables("Tabel").Rows.Count >= 1 Then
                    txtPegawai.EditValue = NullTolong(DS.Tables(0).Rows(0).Item("IDPegawaiRequest"))
                    ...
                    If IsPosted Then
                        txtWilayahDiminta.Properties.ReadOnly = True
    ...
    Public Sub RefreshDetil()
        ...
        Windows.Forms.Cursor.Current = Cursors.WaitCursor
        Try
            strsql = "SELECT MRequestMutasiWilayahD.*, MBarang.Nama AS NamaStock, MBarang.Kode AS KodeStock, MSatuan.Nama AS Satuan, MSatuan.Kode AS KodeSatuan " & vbCrLf & _
                     ...
                     " WHERE MRequestMutasiWilayahD.IDHeader = " & NoID
    ...
    Private Function IsValidasi() As Boolean
        ...
        If txtJam.Text = "" Then
            ...
            Exit Function
        End If
        If CekKodeValid(txtKode.Text, KodeLama, "MRequestMutasiWilayah", "Kode", IIf(pTipe = pStatus.Edit, True, False)) Then
            ...
            If pTipe = pStatus.Baru Then
                If XtraMessageBox.Show("Lakukan pemberian kode baru?", NamaAplikasi, MessageBoxButtons.YesNo, MessageBoxIcon.Information) = Windows.Forms.DialogResult.Yes Then
                    txtKode.Text = clsKode.MintaKodeBaru("REQ", "MRequestMutasiWilayah", txtTgl.DateTime, NullTolong(txtWilayah.EditValue), 5)
                End If
            End If
            txtKode.Focus()
            Return False
    ...
    Public Function Simpan(ByRef PesanSalah As String) As Boolean
        ...
        Try
            If pTipe = pStatus.Baru Then
                NoID = GetNewID("MRequestMutasiWilayah", "NoID")
                SQL = "INSERT INTO [MRequestMutasiWilayah] ([NoID],[Nomor],[Kode],[KodeReff],[IDWilayahDari],[IDWilayahUntuk],[Tanggal],[Jam],[IDPegawaiRequest],[IDUserEntry],[IDUserEdit],[IDUserPosting],[IDAdmin],[TanggalPosting],[JamPosting],[IsPosted]) VALUES (" & vbCrLf & _
                      NoID & ", " & _
                      NullTolong(GetNewID("MRequestMutasiWilayah", "Nomor")) & ", " & _
                      ...
                      "'" & txtTgl.DateTime.ToString("HH:mm") & "', " & _
                      ...
                      "0)"
            Else
                SQL = "UPDATE MRequestMutasiWilayah SET " & vbCrLf & _
                      ...
                      "IDPegawaiRequest=" & NullTolong(txtPegawai.EditValue) & "," & _
                      "IDUserEdit=" & IDUserAktif
    ...
    Private Sub cmdDelete_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdDelete.Click
        ...
        Try
            ...
            If XtraMessageBox.Show("Item " & GV1.GetRowCellValue(GV1.FocusedRowHandle, "NamaStock") & " ini mau dihapus?", NamaAplikasi, MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button2) = Windows.Forms.DialogResult.Yes Then
                EksekusiSQL("Delete From MRequestMutasiWilayahD where NoID=" & IDDetil.ToString)
                RefreshDetil()
            End If
            txtBarang.Focus()
        Catch ex As Exception
    ...
    Private Sub InsertIntoDetil()
        ...
        Dim frmEntri As New frmEntriRequestMutasiWilayahD
        Dim Konversi As Double = 0.0
        Try
            If txtWilayahDiminta.Text <> "" Then
                If CariBarang(IDBarang, NamaBarang, KodeBarang) Then
                    If XtraMessageBox.Show("Ingin menambah request kode barang " & KodeBarang & " dengan nama barang " & NamaBarang, "Fast Entri Says", MessageBoxButtons.YesNo) = Windows.Forms.DialogResult.Yes Then
                        IDDetil = GetNewID("MRequestMutasiWilayahD", "NoID")
                        Konversi = NullToDbl(EksekusiSQlSkalarNew("SELECT Konversi FROM MBarangD WHERE IDBarang=" & IDBarang & " AND IDSatuan=" & DefIDSatuan))
                        If Konversi <> 0 Then
                            SQL = "INSERT INTO MRequestMutasiWilayahD (NoID,IDHeader,IDBarang,IDSatuan,Konversi) VALUES " & vbCrLf
                            SQL &= "(" & IDDetil & "," & NoID & "," & IDBarang & "," & DefIDSatuan & "," & FixKoma(Konversi) & ")"
                        Else
                            SQL = "INSERT INTO MRequestMutasiWilayahD (NoID,IDHeader,IDBarang) VALUES " & vbCrLf
                        ...
                        frmEntri.IDWilayahDari = NullToLong(txtWilayahDiminta.EditValue)
                        ...
                        'Else
                        '    SQL = "DELETE FROM MRequestMutasiWilayahD WHERE NoID=" & IDDetil
                        '    EksekusiSQL(SQL)
                        '    RefreshDetil()
                        '    GV1.ClearSelection()
                        '    GV1.FocusedRowHandle = GV1.LocateByDisplayText(0, GV1.Columns("NoID"), IDDetil.ToString("#,##0"))
