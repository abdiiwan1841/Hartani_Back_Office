...
Public Class frmEntriJual
    ...
    Private Sub SetTombol()
        ...
        If pTipe = pStatus.Baru Or IsPosted Or IsPOS Then
            cmdBAru.Enabled = False
            cmdEdit.Enabled = False
            cmdDelete.Enabled = False
            If IsPOS Then
    ...
    Private Function IsValidasi() As Boolean
        ...
        Try
            ...
            If CekKodeValid(txtKode.Text, KodeLama, "MJual", "Kode", IIf(pTipe = pStatus.Edit, True, False)) Then
                ...
                Exit Function
            End If
            If CekKodeValid(txtNoFakturPajak.Text, FakturPajakLama, "MJual", "NoFakturPajak", IIf(pTipe = pStatus.Edit, True, False)) Then
    ...
    Public Function Simpan(ByRef PesanSalah As String) As Boolean
        ...
        Try
            If pTipe = pStatus.Baru Then
                ...
                SQL &= " Biaya, Total, Bayar, Sisa,PPN,NilaiPPN,DPP,IDAdmin,IDPacking,Keterangan,NoFakturPajak,IDTypePajak) VALUES (" & vbCrLf
                ...
                SQL &= FixApostropi(txtKeterangan.Text) & "', '" & FixApostropi(txtNoFakturPajak.Text) & "'," & NullToLong(TypePajak.EditValue) & " )"
            Else
            ...
            End If
            If Sukses Then
                Dim tempPPN As Double = 0, tempDPP As Double = 0
                If NullToLong(TypePajak.EditValue) = 1 Then 'Sudah Termasuk
                    DS = ExecuteDataset("MJualD", "SELECT * FROM MJualD WHERE IDJual=" & NoID)
                    For i As Integer = 0 To DS.Tables("MJualD").Rows.Count - 1
                        With DS.Tables("MJualD").Rows(i)
                            If i = DS.Tables("MJualD").Rows.Count - 1 Then 'Genapkan dengan Header
                                DPP = txtDPP.EditValue - tempDPP
                                PPN = txtDPP.EditValue - tempPPN
                                tempPPN = tempPPN + PPN
                                tempDPP = tempDPP + DPP
                            Else
                                DPP = Bulatkan(NullToDbl(.Item("Jumlah")) / (1 + (txtPPNProsen.EditValue / 100)), 0)
                                PPN = NullToDbl(.Item("Jumlah")) - DPP
                                tempPPN = tempPPN + PPN
                                tempDPP = tempDPP + DPP
                            End If
                            SQL = "UPDATE MJualD SET PPN=" & FixKoma(PPN) & ", DPP=" & FixKoma(DPP) & " WHERE NoID=" & NullToLong(.Item("NoID"))
                            EksekusiSQL(SQL)
                        End With
                    Next
                ElseIf NullToLong(TypePajak.EditValue) = 2 Then 'Belum Termasuk
                    DS = ExecuteDataset("MJualD", "SELECT * FROM MJualD WHERE IDJual=" & NoID)
                    For i As Integer = 0 To DS.Tables("MJualD").Rows.Count - 1
                        With DS.Tables("MJualD").Rows(i)
                            If i = DS.Tables("MJualD").Rows.Count - 1 Then 'Genapkan dengan Header
                                ...
                                tempDPP = tempDPP + DPP
                            Else
                                DPP = NullToDbl(.Item("Jumlah"))
                                PPN = Bulatkan(NullToDbl(.Item("Jumlah")) * txtPPNProsen.EditValue / 100, 0)
                                tempPPN = tempPPN + PPN
                                tempDPP = tempDPP + DPP
                            End If
                            SQL = "UPDATE MJualD SET PPN=" & FixKoma(PPN) & ", DPP=" & FixKoma(DPP) & " WHERE NoID=" & NullToLong(.Item("NoID"))
                    ...
                    Next
                Else
                    SQL = "UPDATE MJualD SET PPN=0, DPP=0 WHERE IDJual=" & NoID
                    EksekusiSQL(SQL)
                End If
            End If
        Catch ex As Exception
    ...
    Private Sub Tgl_EditValueChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Tgl.EditValueChanged
        ...
        Try
            ...
            If pTipe = pStatus.Baru Then
                If ckCash.Checked Then
                    txtKode.Text = clsKode.MintaKodeBaruPajakKeluaran("MJual", "Kode", Tgl.DateTime, 1, 5, " IsNull(MJual.IsPOS,0)=0 ")
                Else
                    txtKode.Text = clsKode.MintaKodeBaruPajakKeluaran("MJual", "Kode", Tgl.DateTime, 1, 5, " IsNull(MJual.IsPOS,0)=0 ")
                End If
                If FormatFP <> "" AndAlso FormatFP.Length = 19 Then
                    txtNoFakturPajak.Text = FormatFP.Substring(0, 8) & Tgl.DateTime.ToString("yy") & "." & clsKode.MintaKodeBaruFP("MJual", "NoFakturPajak", Tgl.DateTime, 12, 8)
                Else
                    txtNoFakturPajak.Text = "010.000-" & Tgl.DateTime.ToString("yy") & "." & clsKode.MintaKodeBaruFP("MJual", "NoFakturPajak", Tgl.DateTime, 12, 8)
                End If
            End If
        Catch ex As Exception
    ...
    Private Sub InsertIntoDetil()
        ...
        Try
            If CariBarang(IDBarangD, IDBarang, NamaBarang, KodeBarang, Barcode, Konversi, IDSatuan, HargaJual) Then
                'If XtraMessageBox.Show("Ingin menjual item dengan Kode Stock " & KodeBarang & " Nama Stock " & NamaBarang, "Fast Entri Says", MessageBoxButtons.YesNo) = Windows.Forms.DialogResult.Yes Then
                'IDDetil = GetNewID("MJualD", "NoID")
                'SQL = "INSERT INTO MJualD (NoID,IDGudang,IDBarangD,IDBarang,IDSatuan,Konversi,Harga,IDJual,NoUrut,Tgl,Jam) VALUES " & vbCrLf
                'SQL &= "(" & IDDetil & "," & DefIDGudang & "," & IDBarangD & "," & IDBarang & "," & IDSatuan & "," & FixKoma(Konversi) & "," & FixKoma(HargaJual) & "," & NoID & "," & GetNewID("MJualD", "NoUrut", " WHERE IDJual=" & NoID) & ",GetDate(),GetDate())"
                ...
                'If Konversi <> 0 Then
                '    SQL = "INSERT INTO MJualD (NoID,IDGudang,IDBarang,IDSatuan,Konversi,IDJual,NoUrut,Tgl,Jam) VALUES " & vbCrLf
                '    SQL &= "(" & IDDetil & "," & DefIDGudang & "," & IDBarang & "," & DefIDSatuan & "," & FixKoma(Konversi) & "," & NoID & "," & GetNewID("MJualD", "NoUrut", " WHERE IDJual=" & NoID) & ",GetDate(),GetDate())"
                'Else
                '    SQL = "INSERT INTO MJualD (NoID,IDGudang,IDBarang,IDJual,NoUrut,Tgl,Jam) VALUES " & vbCrLf
                '    SQL &= "(" & IDDetil & "," & DefIDGudang & "," & IDBarang & "," & NoID & "," & GetNewID("MJualD", "NoUrut", " WHERE IDJual=" & NoID) & ",GetDate(),GetDate())"
                'End If
                'EksekusiSQL(SQL)
                frmEntri.IsNew = True
    ...
    Private Sub GVPajak_DataSourceChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles GVPajak.DataSourceChanged
        If System.IO.File.Exists(FolderLayouts & Me.Name & GVPajak.Name & ".xml") Then
            GVPajak.RestoreLayoutFromXml(FolderLayouts & Me.Name & GVPajak.Name & ".xml")
        End If
        With GVPajak
