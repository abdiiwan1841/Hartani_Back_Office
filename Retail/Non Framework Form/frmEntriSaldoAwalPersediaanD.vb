...
Public Class frmEntriSaldoAwalPersediaanD
    ...
    Private Sub LoadData()
        Try
            SQL = "SELECT MSaldoAwalPersediaanD.*, MBarang.CtnPcs, MBarang.Kode AS KodeBarang, MBarang.Nama AS NamaBarang, MSatuan.Nama AS NamaSatuan "
            SQL &= " FROM (MSaldoAwalPersediaanD LEFT JOIN MBarang ON MBarang.NoID=MSaldoAwalPersediaanD.IDBarang) "
            SQL &= " LEFT JOIN MSatuan ON MSatuan.NoID=MSaldoAwalPersediaanD.IDSatuan "
            SQL &= " WHERE MSaldoAwalPersediaanD.NoID= " & NoID
            oDS = New DataSet()
    ...
    Private Function Simpan() As Boolean
        Try
            If IsNew Then
                SQL = "INSERT INTO [MSaldoAwalPersediaanD] ([NoID],[IDHeader],[IDBarang],[IDGudang],[IDSatuan],[Konversi],[Qty],[QtyPCS],[Keterangan],[HargaPokok],[Jumlah]) VALUES ("
                SQL &= NullTolong(GetNewID("MSaldoAwalPersediaanD", "NoID")) & ","
                SQL &= IDHeader & ","
                ...
                SQL &= ")"
            Else
                SQL = "UPDATE MSaldoAwalPersediaanD SET "
    ...
    Private Function IsValidasi() As Boolean
        ...
        If AdaDiTransaksiLain() Then
            XtraMessageBox.Show("Transaksi item barang " & txtBarang.Text & " sudah ada di gudang " & txtGudang.Text & ".", NamaAplikasi, MessageBoxButtons.OK, MessageBoxIcon.Information)
            txtGudang.Focus()
            Return False
            Exit Function
        End If
        ...
        Return True
    End Function
    Private Function AdaDiTransaksiLain() As Boolean
        Dim ds As New DataSet
        Try
            SQL = "SELECT * FROM MSaldoAwalPersediaanD WHERE IDBarang=" & NullTolong(txtBarang.EditValue) & " AND IDGudang=" & NullTolong(txtGudang.EditValue) & " AND NoID<>" & NoID
            ds = ExecuteDataset("MSaldoAwalPersediaan", SQL)
    ...
    Private Sub txtBarang_EditValueChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles txtBarang.EditValueChanged
        Dim Ds As New DataSet
        Try
            SQL = "SELECT MBarang.Nama AS NamaBarang, MBarang.HPP FROM MBarang WHERE MBarang.NoID=" & NullTolong(txtBarang.EditValue)
