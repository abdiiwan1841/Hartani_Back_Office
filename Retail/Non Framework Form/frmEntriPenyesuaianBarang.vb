...
Public Class frmEntriPenyesuaianBarang
    Dim SQL As String = ""
    Public Enum pStatus
        Baru = 0
        Edit = 1
    ...
    Private Sub frmEntriPO_FormClosing(ByVal sender As Object, ByVal e As System.Windows.Forms.FormClosingEventArgs) Handles Me.FormClosing
        If IsTempInsertBaru Then
            If XtraMessageBox.Show("Yakin ingin keluar?" & vbCrLf & "Ya, akan menghapus transaksi dan detil item.", NamaAplikasi, MessageBoxButtons.YesNo, MessageBoxIcon.Information, MessageBoxDefaultButton.Button2) = Windows.Forms.DialogResult.Yes Then
                EksekusiSQL("DELETE FROM MPenyesuaian WHERE NoID=" & NoID)
                EksekusiSQL("DELETE FROM MPenyesuaianD WHERE IDHeader=" & NoID)
            Else
                e.Cancel = True
            End If
        End If
    ...
    Private Sub RefreshData()
        ...
        Try
            ...
            If pTipe = pStatus.Baru Then
                IsiDefault()
            Else
                SQL = "SELECT MPenyesuaian.*, MUserEntri.Nama AS UserEntri, MUserEdit.Nama AS UserEdit, MUserPosting.Nama AS UserPosting, MAlamat.Alamat AS Alamat, MAlamat.Nama AS NamaAlamat, MAlamat.Kode as KodeAlamat "
                SQL &= " FROM MPenyesuaian LEFT JOIN MAlamat ON MAlamat.NoID=MPenyesuaian.IDPegawai "
                SQL &= " LEFT JOIN MUser MUserEntri ON MUserEntri.NoID=MPenyesuaian.IDUserEntry "
                SQL &= " LEFT JOIN MUser MUserEdit ON MUserEdit.NoID=MPenyesuaian.IDUserEdit "
                SQL &= " LEFT JOIN MUser MUserPosting ON MUserPosting.NoID=MPenyesuaian.IDUserPosting "
                SQL &= " WHERE MPenyesuaian.NoID = " & NoID
    ...
    Public Sub RefreshDetil()
        ...
        Windows.Forms.Cursor.Current = Cursors.WaitCursor
        Try
            strsql = "SELECT MPenyesuaianD.*, MBarangD.Barcode, MBarangD.Varian AS [Varian/Ukuran], MBarang.Nama, MBarang.Nama + ' ' + IsNull(MBarangD.Varian,'') AS NamaStock, MGudang.Nama AS Gudang, MBarang.Kode AS KodeStock, MSatuan.Nama AS Satuan, MSatuan.Kode AS KodeSatuan " & vbCrLf & _
                     " FROM MPenyesuaianD " & vbCrLf & _
                     " LEFT JOIN MBarangD ON MBarangD.NoID=MPenyesuaianD.IDBarangD" & vbCrLf & _
                     " LEFT JOIN MBarang ON MBarang.NoID=MPenyesuaianD.IDBarang" & vbCrLf & _
                     " LEFT JOIN MGudang ON MGudang.NoID=MPenyesuaianD.IDGudang" & vbCrLf & _
                     " LEFT JOIN MSatuan ON MSatuan.NoID=MPenyesuaianD.IDSatuan" & vbCrLf & _
                     " WHERE MPenyesuaianD.IDHeader = " & NoID
        ...
        If Not IsPosted Then
            If IsValidasi() Then
                ...
                If Simpan(PesanSalah) = True Then
                    ...
                    Else
                        clsPostingMutasiWilayah.PostingPenyesuaian(NoID)
    ...
    Private Function IsValidasi() As Boolean
        ...
        If txtJam.Text = "" Then
            ...
            Exit Function
        End If
        If CekKodeValid(txtKode.Text, KodeLama, "MPenyesuaian", "Kode", IIf(pTipe = pStatus.Edit, True, False)) Then
            ...
            If pTipe = pStatus.Baru Then
                If XtraMessageBox.Show("Lakukan pemberian kode baru?", NamaAplikasi, MessageBoxButtons.YesNo, MessageBoxIcon.Information) = Windows.Forms.DialogResult.Yes Then
                    txtKode.Text = clsKode.MintaKodeBaru("PYS", "MPenyesuaian", txtTgl.DateTime, NullTolong(txtWilayah.EditValue), 5)
                End If
            End If
            txtKode.Focus()
            Return False
    ...
    Public Function Simpan(ByRef PesanSalah As String) As Boolean
        ...
        Try
            If pTipe = pStatus.Baru Then
                NoID = GetNewID("MPenyesuaian", "NoID")
                SQL = "INSERT INTO [MPenyesuaian] ([NoID],[Nomor],[Kode],[KodeReff],[IDWilayah],[Tanggal],[Jam],[IDPegawai],[IDUserEntry],[TglEntri],[IDUserEdit],[IDUserPosting],[IDAdmin],[TglPosting],[IsPosted],[Total],[Keterangan]) VALUES (" & vbCrLf & _
                      NoID & ", " & _
                      NullTolong(GetNewID("MPenyesuaian", "Nomor")) & ", " & _
            ...
            Else
                SQL = "UPDATE MPenyesuaian SET " & vbCrLf & _
    ...
    Private Sub cmdDelete_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdDelete.Click
        ...
        Try
            ...
            If XtraMessageBox.Show("Item " & GV1.GetRowCellValue(GV1.FocusedRowHandle, "NamaStock") & " ini mau dihapus?", NamaAplikasi, MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button2) = Windows.Forms.DialogResult.Yes Then
                EksekusiSQL("Delete From MPenyesuaianD where NoID=" & IDDetil.ToString)
                RefreshDetil()
            End If
            txtBarang.Focus()
        Catch ex As Exception
    ...
    Private Sub InsertIntoDetil()
        ...
        Dim frmEntri As New frmEntriPenyesuaianBarangD
            ...
            If txtWilayah.Text <> "" Then
                If CariBarang(IDBarang, NamaBarang) Then
                    If XtraMessageBox.Show("Ingin menambah barang " & NamaBarang, "Fast Entri Says", MessageBoxButtons.YesNo) = Windows.Forms.DialogResult.Yes Then
                        IDDetil = GetNewID("MPenyesuaianD", "NoID")
                        ...
                        If Konversi <> 0 Then
                            SQL = "INSERT INTO MPenyesuaianD (NoID,IDHeader,IDBarang,IDSatuan,Konversi,HargaPokok) VALUES " & vbCrLf
                            SQL &= "(" & IDDetil & "," & NoID & "," & IDBarang & "," & DefIDSatuan & "," & FixKoma(Konversi) & "," & FixKoma(HargaPokok) & ")"
                        Else
                            SQL = "INSERT INTO MPenyesuaianD (NoID,IDHeader,IDBarang,HargaPokok) VALUES " & vbCrLf
                        ...
                        Else
                            SQL = "DELETE FROM MPenyesuaianD WHERE NoID=" & IDDetil
                            EksekusiSQL(SQL)
                            RefreshDetil()
                            GV1.ClearSelection()
                            GV1.FocusedRowHandle = GV1.LocateByDisplayText(0, GV1.Columns("NoID"), IDDetil.ToString("#,##0"))
