...
Public Class frmEntriTransferINMutasiWilayahD
    Public IsNew As Boolean = True
    Public IsPosted As Boolean = False
    Public NoID As Long = -1
    Public IDHeader As Long = -1
    Public IDWilayahUntuk As Long = -1
    Public IDTransferOut As Long = -1
    ...
    Private Sub IsiDefault()
        ...
        txtGudang.EditValue = NullTolong(EksekusiSQlSkalarNew("SELECT IDGudangDefault FROM MWilayah WHERE NoID=" & IDWilayahUntuk))
    ...
    Private Sub RefreshLookUp()
        ...
        Try
            ...
            SQL = "SELECT MTransferOutD.NoID, MTransferOut.Kode, MBarang.Kode AS KodeStock, MBarang.Nama AS NamaStock, MSatuan.Nama AS Satuan, MTransferOutD.Qty,  MTransferOutD.QtyPcs, MTransferOutD.QtyPcs-" & _
                  " IsNull((SELECT SUM(MTransferIND.QtyPcs) FROM MTransferIND WHERE MTransferIND.NoID<>" & NoID & " AND MTransferIND.IDTransferOutD=MTransferOutD.NoID),0) AS Sisa  " & vbCrLf & _
                  " FROM MTransferOutD " & vbCrLf & _
                  " LEFT JOIN MTransferOut ON MTransferOut.NoID=MTransferOutD.IDHeader " & vbCrLf & _
                  ...
                  " WHERE MTransferOut.NoID=" & IDTransferOut & " AND MTransferOut.IsPosted=1 AND (MTransferOut.IsSelesai=0 OR MTransferOut.IsSelesai IS NULL) AND MBarang.IsActive=1 AND MTransferOutD.QtyPcs-IsNull((SELECT SUM(MTransferIND.QtyPcs) FROM MTransferIND WHERE MTransferIND.NoID<>" & NoID & " AND MTransferIND.IDTransferOutD=MTransferOutD.NoID),0)>0"
            ds = ExecuteDataset("MBarang", SQL)
            txtTransferOut.Properties.DataSource = ds.Tables("MBarang")
            txtTransferOut.Properties.ValueMember = "NoID"
            txtTransferOut.Properties.DisplayMember = "Kode"

            SQL = "SELECT MGudang.NoID, MGudang.Kode, MGudang.Nama, MWilayah.Nama AS Wilayah FROM MGudang LEFT JOIN MWilayah ON MWilayah.NoID=MGudang.IDWilayah WHERE MGudang.IsActive=1 AND MGudang.InTransit=0 AND MGudang.IDWilayah=" & IDWilayahUntuk
    ...
    Private Sub LoadData()
        Try
            SQL = "SELECT MTransferIND.*, MBarang.CtnPcs, MBarang.Kode AS KodeBarang, MBarang.Nama AS NamaBarang, MSatuan.Nama AS NamaSatuan "
            SQL &= " FROM (MTransferIND LEFT JOIN MBarang ON MBarang.NoID=MTransferIND.IDBarang) "
            SQL &= " LEFT JOIN MSatuan ON MSatuan.NoID=MTransferIND.IDSatuan "
            SQL &= " WHERE MTransferIND.NoID= " & NoID
            oDS = New DataSet()
            ...
            If oDS.Tables(0).Rows.Count >= 1 Then
                With oDS.Tables("MPOD").Rows(0)
                    ...
                    txtTransferOut.EditValue = NullTolong(.Item("IDTransferOutD"))
    ...
    Private Function Simpan() As Boolean
        Try
            If IsNew Then
                SQL = "INSERT INTO [MTransferIND] ([NoID],[IDTransferOutD],[IDHeader],[IDBarang],[IDSatuan],[Konversi],[Qty],[QtyPCS],[Ctn],[Keterangan],[IDGudang]) VALUES ("
                SQL &= NullTolong(GetNewID("MTransferIND", "NoID")) & ","
                SQL &= NullTolong(txtTransferOut.EditValue) & ","
                SQL &= IDHeader & ","
                ...
                SQL &= ")"
            Else
                SQL = "UPDATE MTransferIND SET "
                SQL &= " IDTransferOutD=" & NullTolong(txtTransferOut.EditValue) & ","
    ...
    Private Function IsValidasi() As Boolean
        ...
        If txtGudang.Text = "" Then
            XtraMessageBox.Show("Gudang tujuan tidak boleh kosong.", NamaAplikasi, MessageBoxButtons.OK, MessageBoxIcon.Information)
            txtGudang.Focus()
            Return False
            Exit Function
        End If
        ...
        If txtQtyPcs.EditValue < 0 Then
            ...
            Exit Function
        End If
        If txtTransferOut.Text <> "" Then
            If SisaStockTransferOut() < txtQtyPcs.EditValue Then
                XtraMessageBox.Show("Nama stock " & txtNamaStock.Text & " melebihi stock item TransferOut " & txtTransferOut.Text, NamaAplikasi, MessageBoxButtons.OK, MessageBoxIcon.Information)
                txtQty.Focus()
                Return False
                Exit Function
            End If
        Else
            If XtraMessageBox.Show("Item TransferOut masih kosong, ingin melanjutkan penyimpanan?", NamaAplikasi, MessageBoxButtons.YesNo, MessageBoxIcon.Warning, MessageBoxDefaultButton.Button2) = Windows.Forms.DialogResult.No Then
                txtTransferOut.Focus()
                Return False
                Exit Function
            End If
        End If
    ...
    '    End Try
    'End Function
    Private Function SisaStockTransferOut() As Double
        Dim SQL As String = ""
        Dim ds As New DataSet
        Try
            SQL = "SELECT MTransferOutD.QtyPcs-IsNull((SELECT SUM(MTransferIND.QtyPcs) FROM MTransferIND WHERE MTransferIND.IDTransferOutD=MTransferOutD.NoID AND MTransferIND.NoID<>" & NoID & "),0) AS QtyAkhir " & _
                  " FROM MTransferOutD " & _
                  " WHERE MTransferOutD.NoID=" & NullTolong(txtTransferOut.EditValue)
            ds = ExecuteDataset("MStock", SQL)
            If ds.Tables("MStock").Rows.Count >= 1 Then
                Return NullToDbl(ds.Tables(0).Rows(0).Item("QtyAkhir"))
            Else
    ...
    Private Sub BarButtonItem1_ItemClick(ByVal sender As System.Object, ByVal e As DevExpress.XtraBars.ItemClickEventArgs) Handles cmdSaveLayout.ItemClick
        ...
        Try
            If XtraMessageBox.Show("Simpan Layout?", NamaAplikasi, MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button1) = Windows.Forms.DialogResult.Yes AndAlso xOtorisasi.ShowDialog(Me) = Windows.Forms.DialogResult.OK Then
                ...
                gvTransferOut.SaveLayoutToXml(folderLayouts & Me.Name & gvTransferOut.Name & ".xml")
    ...
    '                 " MKategori.Nama AS Kategori, MBarang.HargaPasar, MBarang.HargaJualA, MBarang.HargaJualB, MBarang.HargaJualC, " & vbCrLf & _
    ...
    '        If System.IO.File.Exists(folderLayouts &  Me.Name & gvTransferOut.Name & ".xml") Then
    '            gvTransferOut.RestoreLayoutFromXml(folderLayouts &  Me.Name & gvTransferOut.Name & ".xml")
    '        End If
    '        With gvTransferOut
    ...
    Private Sub gvTransferOut_DataSourceChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles gvTransferOut.DataSourceChanged
        If System.IO.File.Exists(folderLayouts & Me.Name & gvTransferOut.Name & ".xml") Then
            gvTransferOut.RestoreLayoutFromXml(folderLayouts & Me.Name & gvTransferOut.Name & ".xml")
        End If
        With gvTransferOut
    ...
    Private Sub txtTransferOut_EditValueChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles txtTransferOut.EditValueChanged
        Dim Ds As New DataSet
        Dim SisaQty As Double = 0.0
        Try
            SQL = "SELECT MTransferOutD.*, MBarang.Kode AS KodeBarang, MBarang.Nama AS NamaBarang, MSatuan.Nama AS NamaSatuan "
            SQL &= " FROM (MTransferOutD LEFT JOIN MBarang ON MBarang.NoID=MTransferOutD.IDBarang)"
            SQL &= " LEFT JOIN MSatuan ON MSatuan.NoID=MTransferOutD.IDSatuan "
            SQL &= " WHERE MTransferOutD.NoID= " & NullTolong(txtTransferOut.EditValue)
            Ds = New DataSet()
            ...
            If Ds.Tables(0).Rows.Count >= 1 Then
                With Ds.Tables("Master").Rows(0)
                    ...
                    txtSatuan.EditValue = NullTolong(.Item("IDSatuan"))
                    txtKonversi.EditValue = NullToDbl(.Item("Konversi"))
                    SisaQty = NullToDbl(SisaStockTransferOut)
