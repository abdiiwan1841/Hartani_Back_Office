...
Public Class frmEntriSPKMutasiWilayahD
    ...
    Private Sub RefreshLookUp()
        ...
        End Try
    End Sub
    Private Sub RefreshLookUpRequest()
        Dim ds As New DataSet
        Try
            SQL = "SELECT MRequestMutasiWilayahD.NoID, MRequestMutasiWilayah.Kode, MBarang.Kode AS KodeStock, MBarang.Nama AS NamaStock, MSatuan.Nama AS Satuan, MRequestMutasiWilayahD.Qty,  MRequestMutasiWilayahD.QtyPcs, MRequestMutasiWilayahD.QtyPcs-" & _
                  " IsNull((SELECT SUM(MSPKMutasiwilayahD.QtyPcs) FROM MSPKMutasiwilayahD WHERE MSPKMutasiwilayahD.NoID<>" & NoID & " AND MSPKMutasiwilayahD.IDRequestMutasiWilayahD=MRequestMutasiWilayahD.NoID),0) AS Sisa  " & vbCrLf & _
                  " FROM MRequestMutasiWilayahD " & vbCrLf & _
                  " LEFT JOIN MRequestMutasiWilayah ON MRequestMutasiWilayah.NoID=MRequestMutasiWilayahD.IDHeader " & vbCrLf & _
                  " LEFT JOIN MBarang ON MBarang.NoID=MRequestMutasiWilayahD.IDBarang " & vbCrLf & _
                  " LEFT JOIN MSatuan ON MSatuan.NoID=MRequestMutasiWilayahD.IDSatuan " & vbCrLf & _
                  " WHERE MRequestMutasiWilayahD.IDBarang=" & NullToLong(txtBarang.EditValue) & " AND MRequestMutasiWilayah.NoID=" & IDRequest & " AND MRequestMutasiWilayah.IsAccPusat=1 AND MRequestMutasiWilayah.IsPosted=1 AND (MRequestMutasiWilayah.IsSelesai=0 OR MRequestMutasiWilayah.IsSelesai IS NULL) AND MBarang.IsActive=1 AND MRequestMutasiWilayahD.QtyPcs-IsNull((SELECT SUM(MSPKMutasiwilayahD.QtyPcs) FROM MSPKMutasiwilayahD WHERE MSPKMutasiwilayahD.NoID<>" & NoID & " AND MSPKMutasiwilayahD.IDRequestMutasiWilayahD=MRequestMutasiWilayahD.NoID),0)>0"
            ds = ExecuteDataset("MBarang", SQL)
            txtRequest.Properties.DataSource = ds.Tables("MBarang")
            txtRequest.Properties.ValueMember = "NoID"
            txtRequest.Properties.DisplayMember = "Kode"
        Catch ex As Exception
            XtraMessageBox.Show("Kesalahan : " & ex.Message, NamaAplikasi, MessageBoxButtons.OK, MessageBoxIcon.Error)
        Finally
            ds.Dispose()
    ...
    Private Sub LoadData()
        Try
            SQL = "SELECT MSPKMutasiWilayahD.*, MBarang.CtnPcs, MBarang.Kode AS KodeBarang, MBarang.Nama AS NamaBarang, MSatuan.Nama AS NamaSatuan "
            ...
            SQL &= " WHERE MSPKMutasiWilayahD.NoID= " & NoID
            oDS = New DataSet()
            ...
            If oDS.Tables(0).Rows.Count >= 1 Then
                With oDS.Tables("MPOD").Rows(0)
                    ...
                    txtRequest.EditValue = NullTolong(.Item("IDRequestMutasiWilayahD"))
    ...
    Private Function Simpan() As Boolean
        Try
            If IsNew Then
                SQL = "INSERT INTO [MSPKMutasiWilayahD] ([SisaQtyPcs],[NoID],[IDRequestMutasiWilayahD],[IDHeader],[IDBarang],[IDSatuan],[Konversi],[Qty],[QtyPCS],[Ctn],[Keterangan]) VALUES ("
                SQL &= FixKoma(SisaStockRequest() - txtQtyPcs.EditValue) & ","
                SQL &= NullTolong(GetNewID("MSPKMutasiWilayahD", "NoID")) & ","
                SQL &= NullTolong(txtRequest.EditValue) & ","
                SQL &= IDHeader & ","
                ...
                SQL &= ")"
            Else
                SQL = "UPDATE MSPKMutasiWilayahD SET "
                SQL &= " SisaQtyPcs=" & FixKoma(SisaStockRequest() - txtQtyPcs.EditValue) & ","
                SQL &= " IDRequestMutasiWilayahD=" & NullTolong(txtRequest.EditValue) & ","
    ...
    Private Function IsValidasi() As Boolean
        ...
        If txtRequest.Text <> "" Then
            If SisaStockRequest() < txtQtyPcs.EditValue Then
                If XtraMessageBox.Show("Nama stock " & txtNamaStock.Text & " melebihi stock item request " & txtRequest.Text & vbCrLf & "Yakin ingin melanjutkan penyimpanan?", NamaAplikasi, MessageBoxButtons.YesNo, MessageBoxIcon.Warning) = Windows.Forms.DialogResult.No Then
                    txtQty.Focus()
                    Return False
                    Exit Function
                End If
            End If
        Else
            If XtraMessageBox.Show("Item request masih kosong. yakin ingin meneruskan penyimpanan?", NamaAplikasi, MessageBoxButtons.YesNo, MessageBoxIcon.Warning, MessageBoxDefaultButton.Button2) = Windows.Forms.DialogResult.No Then
                txtRequest.Focus()
                Return False
                Exit Function
            End If
    ...
    '    End Try
    'End Function
    Private Function SisaStockRequest() As Double
        Dim SQL As String = ""
        Dim ds As New DataSet
        Try
            SQL = "SELECT MRequestMutasiwilayahD.QtyPcs-IsNull((SELECT SUM(MSPKMutasiWilayahD.QtyPcs) FROM MSPKMutasiWilayahD WHERE MSPKMutasiWilayahD.IDRequestMutasiWilayahD=MRequestMutasiwilayahD.NoID AND MSPKMutasiWilayahD.NoID<>" & NoID & "),0) AS QtyAkhir " & _
                  " FROM MRequestMutasiwilayahD " & _
                  " WHERE MRequestMutasiwilayahD.NoID=" & NullTolong(txtRequest.EditValue)
            ds = ExecuteDataset("MStock", SQL)
            If ds.Tables("MStock").Rows.Count >= 1 Then
                Return NullToDbl(ds.Tables(0).Rows(0).Item("QtyAkhir"))
            Else
    ...
    Private Sub BarButtonItem1_ItemClick(ByVal sender As System.Object, ByVal e As DevExpress.XtraBars.ItemClickEventArgs) Handles cmdSaveLayout.ItemClick
        ...
        Try
            If XtraMessageBox.Show("Simpan Layout?", NamaAplikasi, MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button1) = Windows.Forms.DialogResult.Yes AndAlso xOtorisasi.ShowDialog(Me) = Windows.Forms.DialogResult.OK Then
                ...
                gvRequest.SaveLayoutToXml(folderLayouts & Me.Name & gvRequest.Name & ".xml")
    ...
    '        If System.IO.File.Exists(folderLayouts &  Me.Name & gvRequest.Name & ".xml") Then
    '            gvRequest.RestoreLayoutFromXml(folderLayouts &  Me.Name & gvRequest.Name & ".xml")
    '        End If
    '        With gvRequest
    ...
    Private Sub gvRequest_DataSourceChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles gvRequest.DataSourceChanged
        If System.IO.File.Exists(folderLayouts & Me.Name & gvRequest.Name & ".xml") Then
            gvRequest.RestoreLayoutFromXml(folderLayouts & Me.Name & gvRequest.Name & ".xml")
        End If
        With gvRequest
    ...
    Private Sub txtRequest_EditValueChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles txtRequest.EditValueChanged
        Dim Ds As New DataSet
        Dim SisaQty As Double = 0.0
        Try
            SQL = "SELECT MRequestMutasiWilayahD.*, MBarang.Kode AS KodeBarang, MBarang.Nama AS NamaBarang, MSatuan.Nama AS NamaSatuan "
            SQL &= " FROM (MRequestMutasiWilayahD LEFT JOIN MBarang ON MBarang.NoID=MRequestMutasiWilayahD.IDBarang)"
            SQL &= " LEFT JOIN MSatuan ON MSatuan.NoID=MRequestMutasiWilayahD.IDSatuan "
            SQL &= " WHERE MRequestMutasiWilayahD.NoID= " & NullTolong(txtRequest.EditValue)
            Ds = New DataSet()
            ...
            If Ds.Tables(0).Rows.Count >= 1 Then
                With Ds.Tables("Master").Rows(0)
                    ...
                    txtSatuan.EditValue = NullTolong(.Item("IDSatuan"))
                    txtKonversi.EditValue = NullToDbl(.Item("Konversi"))
                    SisaQty = NullToDbl(SisaStockRequest)
