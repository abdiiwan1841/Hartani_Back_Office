...
Public Class frmLookUpPembelianDetil
    ...
    Public Enum JenisLookUp
        HistoryAlamat = 0
        HistoryBarang = 1
        HistoryPenjualanPromo = 2
    End Enum
    Public LookUp As JenisLookUp
    Public IDBarang As Long = -1
    Public IDSupplier As Long = -1
    'Public TgDari As Date = TanggalSystem
    'Public TgSampai As Date = TanggalSystem
    ...
    Private Sub frmDaftar_FormClosing(ByVal sender As Object, ByVal e As System.Windows.Forms.FormClosingEventArgs) Handles Me.FormClosing
        Try
            ...
            Dim G As Object
            For Each G In MdiParent.MdiChildren
    ...
    Private Sub ctrlDaftar_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        RefreshLookUpBarang()
        RefreshLookUpGudang()
        If LookUp = JenisLookUp.HistoryPenjualanPromo Then
        Else
        ...
        ElseIf LookUp = JenisLookUp.HistoryPenjualanPromo Then
            LabelControlLookUp.Text = "Kode Barang"
            LabelControl4.Text = "Nama Barang"
            Me.Text = "History Barang sesuai Promo Diskon"
            txtBarang.EditValue = NullToLong(IDBarang)
            txtNamaBarang.Text = NullToStr(EksekusiSQlSkalarNew("SELECT Nama FROM MBarang WHERE NoID=" & NullToLong(txtBarang.EditValue)))
            'TglDari.DateTime = TgDari.AddDays(-30)
            'TglSampai.DateTime = TgSampai
        Else
            LabelControlLookUp.Text = "Kode Barang"
            LabelControl4.Text = "Nama Barang"
            Me.Text = "History Supplier sesuai Barang"
            txtBarang.EditValue = NullToLong(IDBarang)
            txtNamaBarang.Text = NullToStr(EksekusiSQlSkalarNew("SELECT Nama FROM MBarang WHERE NoID=" & NullToLong(txtBarang.EditValue)))
        End If
        RefreshData()
    ...
    Private Sub RefreshLookUpBarang()
        ...
        Dim SQL As String = ""
        Try
            SQL = "SELECT MBarangD.NoID,MBarangD.Barcode,MBarang.Kode,MBarang.Nama,MSatuan.Kode Satuan from MBarangD " & _
                      " inner join MBarang On MBarangD.IDBarang=MBarang.NoID left join MSatuan on MBarangD.IDSatuan=MSatuan.NoID "
            ...
            If LookUp = JenisLookUp.HistoryAlamat Or LookUp = JenisLookUp.HistoryPenjualanPromo Then
    ...
    Public Sub RefreshData()
        ...
        Try
            ...
            If LookUp = JenisLookUp.HistoryPenjualanPromo Then
                SQL = "SELECT MJualD.*, MJual.Kode AS KodeTransaksi, MJual.Tanggal,MBarangD.Barcode, MBarang.Nama AS NamaStock, MBarang.Kode AS KodeStock, MBarangD.Barcode, MJual.NamaKasir, MPOS.Nama AS Kassa, " & _
                      " MGudang.Nama AS Gudang, MWilayah.Nama AS Wilayah, " & _
                      " MAlamat.Nama AS Customer, MSatuan.Nama AS Satuan " & _
                      " FROM (MJualD " & _
                      " INNER JOIN MJual ON MJual.NoID=MJualD.IDJual) " & _
                      " LEFT JOIN MBarang ON MJualD.IDBarang=MBarang.NoID " & _
                      " LEFT JOIN MBarangD ON MJualD.IDBarangD=MBarangD.NoID " & _
                      " LEFT JOIN MPOS ON MJual.IDPOS=MPOS.NoID " & _
                      " LEFT JOIN MSatuan ON MJualD.IDSatuan=MSatuan.NoID " & _
                      " LEFT JOIN (MGudang LEFT JOIN MWilayah ON MWilayah.NoID=MGudang.IDWilayah) ON MJualD.IDGudang=MGudang.NoID " & _
                      " LEFT JOIN MAlamat ON MJual.IDCustomer=MAlamat.NoID "
                IsPerluWhere = IsPerluWhere
                If Not IsSupervisor Then
                    SQL &= IIf(IsPerluWhere, " WHERE ", " AND") & " MWilayah.NoID=" & DefIDWilayah & vbCrLf
                    IsPerluWhere = False
                End If
                If TglDari.Enabled Then
                    SQL &= IIf(IsPerluWhere, " WHERE ", " AND") & " (MJual.Tanggal>='" & TglDari.DateTime.ToString("yyyy/MM/dd") & "' AND MJual.Tanggal<'" & DateAdd(DateInterval.Day, 1, TglSampai.DateTime).ToString("yyyy/MM/dd") & "')" & vbCrLf
                    IsPerluWhere = False
                End If
                If txtBarang.Enabled Then
                    SQL &= IIf(IsPerluWhere, " WHERE ", " AND") & " MBarang.NoID=" & NullToLong(txtBarang.EditValue) & vbCrLf
                    IsPerluWhere = False
                End If
                'If txtBarcode.Enabled Then
                '    SQL &= IIf(IsPerluWhere, " WHERE ", " AND") & " MBarangD.NoID=" & NullToLong(txtBarcode.EditValue) & vbCrLf
                '    IsPerluWhere = False
                'End If
                SQL &= " ORDER BY MJual.Tanggal DESC, MJual.NoID DESC"
            Else
                SQL = "SELECT MBeliD.*, (CASE WHEN MPO.IDTypePajak=1 THEN 'IN' WHEN MPO.IDTypePajak=2 THEN 'EX' ELSE 'NON' END) AS TipePPN, MBeli.Kode AS KodeTransaksi, MBeli.Tanggal, MBarang.Nama AS NamaStock, MBarang.Kode AS KodeStock, " & _
                      " MGudang.Nama AS Gudang, MWilayah.Nama AS Wilayah, " & _
                      " MAlamat.Nama AS Supplier, MSatuan.Nama AS Satuan " & _
                      " FROM (MBeliD " & _
                      " INNER JOIN MBeli ON MBeli.NoID=MBeliD.IDBeli) " & _
                      " LEFT JOIN (MPO INNER JOIN MPOD ON MPO.NoID=MPOD.IDPO) ON MPOD.NoID=MBeliD.IDPOD " & _
                      " LEFT JOIN MBarang ON MBeliD.IDBarang=MBarang.NoID " & _
                      " LEFT JOIN MSatuan ON MBeliD.IDSatuan=MSatuan.NoID " & _
                      " LEFT JOIN (MGudang LEFT JOIN MWilayah ON MWilayah.NoID=MGudang.IDWilayah) ON MBeli.IDGudang=MGudang.NoID " & _
                      " LEFT JOIN MAlamat ON MBeli.IDSupplier=MAlamat.NoID "
                IsPerluWhere = IsPerluWhere
                If Not IsSupervisor Then
                    SQL &= IIf(IsPerluWhere, " WHERE ", " AND") & " MWilayah.NoID=" & DefIDWilayah & vbCrLf
                    IsPerluWhere = False
                End If
                If TglDari.Enabled Then
                    SQL &= IIf(IsPerluWhere, " WHERE ", " AND") & " (MBeli.Tanggal>='" & TglDari.DateTime.ToString("yyyy/MM/dd") & "' AND MBeli.Tanggal<'" & DateAdd(DateInterval.Day, 1, TglSampai.DateTime).ToString("yyyy/MM/dd") & "')" & vbCrLf
                    IsPerluWhere = False
                End If
                ...
                End If
                If LookUp = JenisLookUp.HistoryBarang Then
                    SQL &= IIf(IsPerluWhere, " WHERE ", " AND") & " (MAlamat.NoID=" & NullToLong(txtBarang.EditValue) & ")" & vbCrLf
                    IsPerluWhere = False
                Else
                    SQL &= IIf(IsPerluWhere, " WHERE ", " AND") & " MBarang.NoID=" & NullToLong(txtBarang.EditValue) & vbCrLf
                    IsPerluWhere = False
                End If
                SQL &= " ORDER BY MBeli.Tanggal DESC, MBeli.NoID DESC"
            End If
    ...
    Private Sub CetakKartuStok()
        Dim file As String = Application.StartupPath & "\Report\HistoryPenjualan.rpt"
        Dim SaldoAwal As Double, SaldoAwalD As Double

        Try
            SaldoAwal = NullToDbl(EksekusiSQlSkalarNew("SELECT SUM((MKartuStok.QtyMasuk-MKartuStok.QtyKeluar)*MKartuStok.Konversi) FROM MKartuStok WHERE MKartuStok.IDBarangD IN (SELECT NoID FROM MBarangD WHERE IDBarang=" & NullToLong(txtBarang.EditValue) & ") AND MKartuStok.IDBarang=" & NullToLong(txtBarang.EditValue) & " AND MKartuStok.Tanggal<'" & TglDari.DateTime.AddDays(1).ToString("yyyy/MM/dd") & "'"))
            SaldoAwalD = NullToDbl(EksekusiSQlSkalarNew("SELECT SUM((MKartuStok.QtyMasuk-MKartuStok.QtyKeluar)*MKartuStok.Konversi) FROM MKartuStok WHERE MKartuStok.IDBarangD=" & NullToLong(txtBarang.EditValue) & " AND MKartuStok.IDBarang=" & NullToLong(txtBarang.EditValue) & " AND MKartuStok.Tanggal<'" & TglDari.DateTime.AddDays(1).ToString("yyyy/MM/dd") & "'"))
            mdlCetakCR.ViewReport(Me.MdiParent, IIf(CType(Me.MdiParent, frmMain).ckEditRpt.Checked, mdlCetakCR.action_.Edit, mdlCetakCR.action_.Preview), file, "Kartu Stok", "", "", "IDBarangD=" & NullToLong(txtBarcode.EditValue) & "&SaldoAwalBarangD=" & FixKoma(SaldoAwalD) & "&IDBarang=" & NullToLong(txtBarang.EditValue) & "&SaldoAwal=" & FixKoma(SaldoAwal) & "&TglDari=cdate(" & TglDari.DateTime.ToString("yyyy,MM,dd") & ")&TglSampai=cdate(" & TglSampai.DateTime.ToString("yyyy,MM,dd") & ")")
        Catch ex As Exception
    ...
    Private Sub SimpleButton5_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles SimpleButton5.Click
        ...
        Else
            ...
            Try
                ...
                Dim IDDetil As Long = NullToLong(row("IDBeli"))
                x.NoID = IDDetil
    ...
    Private Sub ckIsSPK_CheckedChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ckAllBarang.CheckedChanged
    ...
    Private Sub txtBarcode_EditValueChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles txtBarcode.EditValueChanged
        If Not IsLoaded Then Exit Sub
        If LookUp = JenisLookUp.HistoryAlamat Or LookUp = JenisLookUp.HistoryPenjualanPromo Then
            'txtNamaBarang.Text = NullToStr(EksekusiSQlSkalarNew("SELECT MBarang.Nama FROM MBarang inner join Mbarangd on mbarangd.idbarang=mbarang.noid WHERE MBarangd.NoID=" & NullToLong(txtBarcode.EditValue)))
            txtBarang.EditValue = NullToLong(EksekusiSQlSkalarNew("SELECT MBarangd.IDBarang FROM MBarangD WHERE MBarangd.NoID=" & NullToLong(txtBarcode.EditValue)))
        ElseIf LookUp = JenisLookUp.HistoryBarang Then
            txtNamaBarang.Text = NullToStr(EksekusiSQlSkalarNew("SELECT Nama FROM MBarang WHERE NoID=" & NullToLong(txtBarang.EditValue)))
        End If
        RefreshData()
    ...
    Private Sub LabelControl7_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles LabelControl7.Click
        txtBarcode.Enabled = Not txtBarcode.Enabled
    End Sub
End Class