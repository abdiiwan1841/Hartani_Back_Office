...
Public Class frmEntriPemakaianBarangD
    ...
    Private Sub LoadData()
        Try
            SQL = "SELECT MPemakaianD.*, MBarang.CtnPcs, MBarang.Kode AS KodeBarang, MBarang.Nama AS NamaBarang, MSatuan.Nama AS NamaSatuan "
            SQL &= " FROM (MPemakaianD LEFT JOIN MBarang ON MBarang.NoID=MPemakaianD.IDBarang) "
            SQL &= " LEFT JOIN MSatuan ON MSatuan.NoID=MPemakaianD.IDSatuan "
            SQL &= " WHERE MPemakaianD.NoID= " & NoID
            oDS = New DataSet()
    ...
    Private Function Simpan() As Boolean
        Try
            If IsNew Then
                SQL = "INSERT INTO [MPemakaianD] ([NoID],[IDHeader],[IDBarang],[IDGudang],[IDSatuan],[Konversi],[Qty],[QtyPCS],[Keterangan],[HargaPokok],[Jumlah],[IDBarangD]) VALUES ("
                SQL &= NullTolong(GetNewID("MPemakaianD", "NoID")) & ","
                SQL &= IDHeader & ","
