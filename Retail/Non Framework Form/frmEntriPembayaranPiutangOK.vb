...
Public Class frmEntriPembayaranPiutangOK
    ...
    Public Sub RefreshDetil()
        ...
        Try
            ...
            Try
                ...
                If Not IsJual Then
                    strsql = "Select TBayarhutangD.*, MBeli.Kode as NoNota,Mbeli.Tanggal,MBeli.Total From TBayarhutangD inner join MBeli on TBayarHutangD.IDBeli=MBeli.NoID where IsJual=" & IIf(IsJual, 1, 0) & " and  TBayarHutangD.IDUser=" & IDUserAktif
                Else
                    strsql = "Select TBayarhutangD.*, MJual.KodeReff as NoNota, MJual.IDPOS AS Kassa, MJual.Tanggal,MJual.Total From TBayarhutangD inner join MJual on TBayarHutangD.IDBeli=MJual.NoID where IsJual=" & IIf(IsJual, 1, 0) & " and  TBayarHutangD.IDUser=" & IDUserAktif
                End If
                ...
                'Retur
                If Not IsJual Then
                    strsql = "Select TBayarhutangDRetur.*,MReturBeli.Kode,MReturBeli.Tanggal,MReturBeli.Total From TBayarhutangDRetur inner join MReturBeli on TBayarhutangDRetur.IDReturBeli=MReturBeli.NoID where TBayarHutangDRetur.IsJual=" & IIf(IsJual, 1, 0) & " and  TBayarHutangDRetur.IDUser=" & IDUserAktif
                Else
                    strsql = "Select TBayarhutangDRetur.*,MReturJual.Kode,MReturJual.Tanggal,MReturJual.Total From TBayarhutangDRetur inner join MReturJual on TBayarhutangDRetur.IDReturBeli=MReturJual.NoID where TBayarHutangDRetur.IsJual=" & IIf(IsJual, 1, 0) & " and  TBayarHutangDRetur.IDUser=" & IDUserAktif
                End If
    ...
    Private Sub SimpanDetil()
        Try
            ...
            For i = 0 To gvPembelian.RowCount - 1
                ...
                If Bayar <> 0 Then
                    EksekusiSQL("Insert Into MBayarHutangD(NoID,IDBayarHutang,IDBeli,Bayar,IsJual) Values(" & _
            ...
            For i = 0 To gvRetur.RowCount - 1
                ...
                If Bayar <> 0 Then
                    EksekusiSQL("Insert Into MBayarHutangDRetur(NoID,IDBayarHutang,IDReturBeli,Potong,IsJual) Values(" & _
                    IDDetil & "," & NoID & "," & IDBeli & "," & FixKoma(Bayar) & "," & IIf(IsJual, 1, 0) & ")")
                End If
            Next
        ...
        Try
            If pTipe = pStatus.Baru Then
                ...
                SQL = "INSERT INTO [MBayarHutang] ([NoID],[Kode],[Tanggal],[IDALamat],[IDDepartemen],[IDWilayah],[Keterangan],[IDUserEntry],[TglEntry],[IDUserEdit],[TglEdit],[IsEdit],[JumlahBeli],[JumlahRetur],[JumlahPH],[NotaDebet],[NotaKredit],[Subtotal],[JumlahUangMuka],[Total],[Catatan],[IsSupplier],[IsJual],[IDKwitansi],[JumlahKwitansi],[Potongan],[Materai],[DN]) VALUES (" & vbCrLf & _
                      ...
                      "'" & FixApostropi(txtCatatan.Text) & "'," & _
                      IIf(IsJual, 0, 1) & "," & _
                      IIf(IsJual, 1, 0) & "," & NullToLong(txtKwitansi.EditValue) & "," & FixKoma(txtJumlahKwitansi.EditValue) & "," & FixKoma(NullToDbl(txtPotongan.EditValue)) & "," & FixKoma(NullToDbl(txtMaterai.EditValue)) & "," & FixKoma(txtDN.EditValue) & ")"
            Else
    ...
    Sub InsertkanKeDetil()
        ...
        If NullToLong(txtKodeAlamat.EditValue) > 0 Then
            If IsNew Then
                'Penjualan
                EksekusiSQL("insert Into TBayarHutangD(IsJual,IDBeli,IDUser) " & _
                "Select " & IIf(IsJual, 1, 0) & " as IsJual, MJual.NoID," & IDUserAktif & " As IDUser " & _
                "FROM MJual " & _
                " Left Join " & _
                " (Select IDBeli,Sum(Bayar) as Bayar FROM MBayarHutangD where IsJual=1 group by IDBeli) " & _
                " B on MJual.NoID=B.IDBeli " & _
                " WHERE MJual.isPosted=1 and MJual.Total-MJual.Kas-MJual.Bank-ISNULL(B.Bayar,0)>0 and MJual.IDCustomer=" & txtKodeAlamat.EditValue)

                'Retur Jual
                EksekusiSQL("insert Into TBayarHutangDRetur(IsJual,IDReturBeli,IDUser) " & _
                                "Select " & IIf(IsJual, 1, 0) & " as IsJual, MReturJual.NoID," & IDUserAktif & " As IDUser " & _
                                "FROM (MReturJual " & _
                                "Left Join " & _
                                "(Select IDReturBeli,Sum(Potong) as Bayar FROM MBayarHutangDRetur where IsJual=1 group by IDReturBeli) " & _
                                "B on MReturJual.NoID=B.IDReturBeli) " & _
                                "WHERE MReturJual.isPosted=1 and MReturJual.Total-ISNULL(B.Bayar,0)<>0 and MReturJual.IDCustomer=" & txtKodeAlamat.EditValue)
                ''Revisi harga Jual
                'EksekusiSQL("insert Into TBayarHutangDPH(IsJual,IDRevisiHarga,IDUser) " & _
                '                "Select " & IIf(IsJual, 1, 0) & " as IsJual, MRevisiHargaJual.NoID," & IDUserAktif & " As IDUser " & _
                '                "FROM ((MRevisiHargaJual inner join " & _
                '                "(select IDRevisiHargaJual,Sum(KoreksiBL) as Total from MRevisiHargaJualD group by IDRevisiHargaJual) MPHD " & _
                '                "on MRevisiHargaJual.NoID=MPHD.IDRevisiHargaJual inner join (select IDTransaksi   from MTTD  " & _
                '                "inner join MTT on MTTD.IDTT=MTT.NoID " & _
                '                "where (MTT.IDCustomer= " & txtKodeAlamat.EditValue & " And MTTD.IDJenisTransaksi = 11) " & _
                '                ") KB on MRevisiHargaJual.NoID=KB.IDTransaksi) " & _
                '                " Left Join " & _
                '                "(Select IDRevisiHarga,Sum(Potong) as Bayar FROM MBayarHutangDPH where IsJual=1 group by IDRevisiHarga) " & _
                '                "B on MRevisiHargaJual.NoID=B.IDRevisiHarga) " & _
                '                "WHERE MRevisiHargaJual.isPosted=1 and MPHD.Total-ISNULL(B.Bayar,0)<>0 and MRevisiHargaJual.IDCustomer=" & txtKodeAlamat.EditValue)
                ''Debet Note
                'EksekusiSQL("insert Into TBayarHutangDDebet(IsJual,IDDebetNote,IDUser) " & _
                '                "Select " & IIf(IsJual, 1, 0) & " as IsJual, MDebetNote.NoID," & IDUserAktif & " As IDUser " & _
                '                "FROM (MDebetNote Left Join " & _
                '                "(Select IDDebetNote,Sum(Potong) as Bayar FROM MBayarHutangDDebet where IsJual=1 group by IDDebetNote) " & _
                '                "B on MDebetNote.NoID=B.IDDebetNote) " & _
                '                "WHERE MDebetNote.isPosted=1 and MDebetNote.Jumlah-ISNULL(B.Bayar,0)<>0 and MDebetNote.IDAlamat=" & txtKodeAlamat.EditValue)

                ''Kredit Note
                'EksekusiSQL("insert Into TBayarHutangDKredit(IsJual,IDKreditNote,IDUser) " & _
                '                "Select " & IIf(IsJual, 1, 0) & " as IsJual, MCreditNote.NoID," & IDUserAktif & " As IDUser " & _
                '                "FROM (MCreditNote Left Join " & _
                '                "(Select IDKreditNote,Sum(Potong) as Bayar FROM MBayarHutangDKredit where IsJual=1 group by IDKreditNote) " & _
                '                "B on MCreditNote.NoID=B.IDKreditNote) " & _
                '                "WHERE MCreditNote.isPosted=1 and MCreditNote.Jumlah-ISNULL(B.Bayar,0)<>0 and MCreditNote.IDAlamat=" & txtKodeAlamat.EditValue)

            Else
                'C=Nota yang Diedit
                'B=Jual
                'RB Retur Jual
                EksekusiSQL("insert Into TBayarHutangD(Isjual,IDBeli,IDUser,Bayar) " & _
                       "Select " & IIf(IsJual, 1, 0) & " as IsJual, MJual.NoID," & IDUserAktif & " As IDUser,C.TerBayar " & _
                       " FROM MJual " & _
                       " Left Join (Select IDBeli,Sum(Bayar) as Bayar FROM MBayarHutangD where IsJual=1 and MBayarHutangD.IDBayarHutang<>" & NoID & " group by IDBeli) B on MJual.NoID=B.IDBeli " & _
                       " LEFT JOIN (select MBayarHutangD.IDBeli,MBayarHutangD.Bayar as TerBayar from MBayarHutangD where MBayarHutangD.IDBayarHutang=" & NoID & ") C on MJual.NoID=C.IDBeli " & _
                       " WHERE MJual.isPosted=1 and  MJual.Total-MJual.Kas-MJual.Bank-Isnull(B.Bayar,0)>0 and MJual.IDCustomer=" & txtKodeAlamat.EditValue)


                'Retur PemJualan
                EksekusiSQL("insert Into TBayarHutangDRetur(Isjual,IDReturBeli,IDUser,Potong) " & _
                       "Select " & IIf(IsJual, 1, 0) & " as IsJual, MReturJual.NoID," & IDUserAktif & " As IDUser,C.TerBayar " & _
                       "FROM (MReturJual Left Join (Select IDReturBeli,Sum(Potong) as Bayar FROM MBayarHutangDRetur where IsJual=" & IIf(IsJual, 1, 0) & " and MBayarHutangDRetur.IDBayarHutang<>" & NoID & " group by IDReturBeli) B on MReturJual.NoID=B.IDReturBeli) " & _
                       "LEFT JOIN (select MBayarHutangDRetur.IDReturBeli,MBayarHutangDRetur.Potong as TerBayar from MBayarHutangDRetur where MBayarHutangDRetur.IDBayarHutang=" & NoID & ") C on MReturJual.NoID=C.IDReturBeli " & _
                       "WHERE MReturJual.isPosted=1 and  MReturJual.Total-Isnull(B.Bayar,0)<>0 and MReturJual.IDCustomer=" & txtKodeAlamat.EditValue)


                ''Revisi Harga Jual
                'EksekusiSQL("insert Into TBayarHutangDPH(Isjual,IDRevisiHarga,IDUser,Potong) " & _
                '       "Select " & IIf(IsJual, 1, 0) & " as IsJual, MRevisiHargaJual.NoID," & IDUserAktif & " As IDUser,C.TerBayar " & _
                '       "FROM ((MRevisiHargaJual inner join " & _
                '                "(select IDRevisiHargaJual,Sum(KoreksiBL) as Total from MRevisiHargaJualD group by IDRevisiHargaJual) MPHD " & _
                '                "on MRevisiHargaJual.NoID=MPHD.IDRevisiHargaJual inner join (select IDTransaksi   from MTTD  " & _
                '        "inner join MTT on MTTD.IDTT=MTT.NoID " & _
                '        "where (MTT.IDCustomer= " & txtKodeAlamat.EditValue & " And MTTD.IDJenisTransaksi = 11) " & _
                '        ") KB on MRevisiHargaJual.NoID=KB.IDTransaksi) Left Join (Select IDRevisiHarga,Sum(Potong) as Bayar FROM MBayarHutangDPH where IsJual=" & IIf(IsJual, 1, 0) & " and MBayarHutangDPH.IDBayarHutang<>" & NoID & " group by IDRevisiHarga) B on MRevisiHargaJual.NoID=B.IDRevisiHarga) " & _
                '       "LEFT JOIN (select MBayarHutangDPH.IDRevisiHarga,MBayarHutangDPH.Potong as TerBayar from MBayarHutangDPH where MBayarHutangDPH.IDBayarHutang=" & NoID & ") C on MRevisiHargaJual.NoID=C.IDRevisiHarga " & _
                '       "WHERE MRevisiHargaJual.isPosted=1 and  MPHD.Total-Isnull(B.Bayar,0)<>0 and MRevisiHargaJual.IDCustomer=" & txtKodeAlamat.EditValue)
    ...
    Private Sub txtNoTT_EditValueChanged(ByVal sender As Object, ByVal e As System.EventArgs)
        'InsertkanKeDetil()
    End Sub
