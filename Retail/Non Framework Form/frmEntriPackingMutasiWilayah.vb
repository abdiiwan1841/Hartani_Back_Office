...
Public Class frmEntriPackingMutasiWilayah
    Dim SQL As String = ""
    Public Enum pStatus
        Baru = 0
        Edit = 1
    ...
    Private Sub frmEntriPO_FormClosing(ByVal sender As Object, ByVal e As System.Windows.Forms.FormClosingEventArgs) Handles Me.FormClosing
        ...
        If IsTempInsertBaru Then
            If XtraMessageBox.Show("Yakin ingin keluar?" & vbCrLf & "Ya, akan menghapus transaksi dan detil item.", NamaAplikasi, MessageBoxButtons.YesNo, MessageBoxIcon.Information, MessageBoxDefaultButton.Button2) = Windows.Forms.DialogResult.Yes Then
                EksekusiSQL("DELETE FROM MPackingMutasiWilayah WHERE NoID=" & NoID)
                EksekusiSQL("DELETE FROM MPackingMutasiWilayahD WHERE IDHeader=" & NoID)
            Else
                e.Cancel = True
            End If
        End If
    ...
    Private Sub RefreshData()
        ...
        Try
            ...
            If pTipe = pStatus.Baru Then
                IsiDefault()
            Else
                SQL = "SELECT MPackingMutasiWilayah.*, MUserEntri.Nama AS UserEntri, MUserEdit.Nama AS UserEdit, MUserPosting.Nama AS UserPosting, MAlamat.Alamat AS Alamat, MAlamat.Nama AS NamaAlamat, MAlamat.Kode as KodeAlamat "
                SQL &= " FROM MPackingMutasiWilayah LEFT JOIN MAlamat ON MAlamat.NoID=MPackingMutasiWilayah.IDPegawai "
                SQL &= " LEFT JOIN MUser MUserEntri ON MUserEntri.NoID=MPackingMutasiWilayah.IDUserEntry "
                SQL &= " LEFT JOIN MUser MUserEdit ON MUserEdit.NoID=MPackingMutasiWilayah.IDUserEdit "
                SQL &= " LEFT JOIN MUser MUserPosting ON MUserPosting.NoID=MPackingMutasiWilayah.IDUserPosting "
                SQL &= " WHERE MPackingMutasiWilayah.NoID = " & NoID
                ...
                If DS.Tables("Tabel").Rows.Count >= 1 Then
                    ...
                    txtKodeReff.EditValue = NullTolong(DS.Tables(0).Rows(0).Item("IDSPKMutasiWilayah"))
    ...
    Public Sub RefreshDetil()
        ...
        Windows.Forms.Cursor.Current = Cursors.WaitCursor
        Try
            strsql = "SELECT MPackingMutasiWilayahD.*, MSPKMutasiWilayah.NoID AS IDSPKMutasi, MSPKMutasiWilayah.Kode AS NoSPKMutasi, MBarang.Nama AS NamaStock, MBarang.Kode AS KodeStock, MSatuan.Nama AS Satuan, MSatuan.Kode AS KodeSatuan " & vbCrLf & _
                     " FROM MPackingMutasiWilayahD " & vbCrLf & _
                     " LEFT JOIN (MSPKMutasiWilayahD LEFT JOIN MSPKMutasiWilayah ON MSPKMutasiWilayah.NoID=MSPKMutasiWilayahD.IDHeader) ON MSPKMutasiWilayahD.NoID=MPackingMutasiWilayahD.IDSPKMutasiWilayahD" & vbCrLf & _
                     " LEFT JOIN MBarang ON MBarang.NoID=MPackingMutasiWilayahD.IDBarang" & vbCrLf & _
                     " LEFT JOIN MSatuan ON MSatuan.NoID=MPackingMutasiWilayahD.IDSatuan" & vbCrLf & _
                     " WHERE MPackingMutasiWilayahD.IDHeader = " & NoID
                ...
                If Simpan(PesanSalah) = True Then
                    If pTipe = pStatus.Baru Then
                        ...
                        If GV1.RowCount = 0 Then
                            InsertIntoDetilPacking(txtKodeReff.EditValue)
                        End If
    ...
    Private Function IsValidasi() As Boolean
        ...
        If Not Sukses Then
            ...
            Exit Function
        End If
        If CekKodeValid(txtKode.Text, KodeLama, "MPackingMutasiWilayah", "Kode", IIf(pTipe = pStatus.Edit, True, False)) Then
            ...
            If pTipe = pStatus.Baru Then
                If XtraMessageBox.Show("Lakukan pemberian kode baru?", NamaAplikasi, MessageBoxButtons.YesNo, MessageBoxIcon.Information) = Windows.Forms.DialogResult.Yes Then
                    txtKode.Text = clsKode.MintaKodeBaru("SPCW", "MPackingMutasiWilayah", txtTgl.DateTime, NullTolong(txtWilayahDiminta.EditValue), 5)
                End If
            End If
            txtKode.Focus()
            Return False
    ...
    Public Function Simpan(ByRef PesanSalah As String) As Boolean
        ...
        Try
            If pTipe = pStatus.Baru Then
                NoID = GetNewID("MPackingMutasiWilayah", "NoID")
                SQL = "INSERT INTO [MPackingMutasiWilayah] ([NoID],[IDSPKMutasiWilayah],[IDRequestMutasiWilayah],[Nomor],[Kode],[KodeReff],[IDWilayahDari],[IDWilayahUntuk],[Tanggal],[Jam],[IDGudangDari],[IDPegawai],[IDUserEntry],[TglEntri],[IDUserEdit],[IDUserPosting],[IDAdmin],[TglPosting],[IsPosted]) VALUES (" & vbCrLf & _
                      NoID & ", " & _
                      NullTolong(txtKodeReff.EditValue) & "," & _
                      NullTolong(EksekusiSQlSkalarNew("SELECT IDRequestMutasiWilayah FROM MSPKMutasiWilayah WHERE NoID=" & NullTolong(txtKodeReff.EditValue))) & "," & _
                      NullTolong(GetNewID("MPackingMutasiWilayah", "Nomor")) & ", " & _
                      ...
                      "0)"
            Else
                SQL = "UPDATE MPackingMutasiWilayah SET " & vbCrLf & _
                      "Kode='" & FixApostropi(txtKode.Text) & "', " & _
                      "IDSPKMutasiWilayah=" & NullTolong(txtKodeReff.EditValue) & ", " & _
                      "KodeReff='" & FixApostropi(txtKodeReff.Text) & "', " & _
                      "IDRequestMutasiWilayah=" & NullTolong(EksekusiSQlSkalarNew("SELECT IDRequestMutasiWilayah FROM MSPKMutasiWilayah WHERE NoID=" & NullTolong(txtKodeReff.EditValue))) & "," & _
    ...
    Private Sub cmdDelete_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdDelete.Click
        ...
        Try
            ...
            If XtraMessageBox.Show("Item " & GV1.GetRowCellValue(GV1.FocusedRowHandle, "NamaStock") & " ini mau dihapus?", NamaAplikasi, MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button2) = Windows.Forms.DialogResult.Yes Then
                EksekusiSQL("Delete From MPackingMutasiWilayahD where NoID=" & IDDetil.ToString)
                RefreshDetil()
            End If
            txtBarang.Focus()
        Catch ex As Exception
    ...
    Private Sub InsertIntoDetil()
        ...
        Dim frmEntri As New frmEntriPackingMutasiWilayahD
            ...
            If txtWilayahDiminta.Text <> "" Then
                If CariBarang(IDBarang, NamaBarang) Then
                    If XtraMessageBox.Show("Ingin menambah barang " & NamaBarang, "Fast Entri Says", MessageBoxButtons.YesNo) = Windows.Forms.DialogResult.Yes Then
                        IDDetil = GetNewID("MPackingMutasiWilayahD", "NoID")
                        Konversi = NullToDbl(EksekusiSQlSkalarNew("SELECT Konversi FROM MBarangD WHERE IDBarang=" & IDBarang & " AND IDSatuan=" & DefIDSatuan))
                        If Konversi <> 0 Then
                            SQL = "INSERT INTO MPackingMutasiWilayahD (NoID,IDHeader,IDBarang,IDSatuan,Konversi) VALUES " & vbCrLf
                            SQL &= "(" & IDDetil & "," & NoID & "," & IDBarang & "," & DefIDSatuan & "," & FixKoma(Konversi) & ")"
                        Else
                            SQL = "INSERT INTO MPackingMutasiWilayahD (NoID,IDHeader,IDBarang) VALUES " & vbCrLf
                        ...
                        frmEntri.IDSPK = NullTolong(txtKodeReff.EditValue)
                        ...
                        Else
                            SQL = "DELETE FROM MPackingMutasiWilayahD WHERE NoID=" & IDDetil
                            EksekusiSQL(SQL)
                            RefreshDetil()
                            GV1.ClearSelection()
                            GV1.FocusedRowHandle = GV1.LocateByDisplayText(0, GV1.Columns("NoID"), IDDetil.ToString("#,##0"))
    ...
    Private Sub RubahLookUp()
        ...
        Try
            SQL = "SELECT NoID, Kode, Tanggal, SUM(Sisa) AS Sisa  " & vbCrLf & _
                  " FROM" & vbCrLf & _
                  " (SELECT MSPKMutasiWilayah.NoID, MSPKMutasiWilayah.Kode, MSPKMutasiWilayah.Tanggal, MSPKMutasiWilayahD.QtyPcs-IsNull((SELECT SUM(MPackingMutasiWilayahD.QtyPcs) FROM MPackingMutasiWilayahD WHERE MPackingMutasiWilayahD.IDSPKMutasiWilayahD=MSPKMutasiWilayahD.NoID),0) AS Sisa " & vbCrLf & _
                  " FROM MSPKMutasiWilayahD LEFT JOIN MSPKMutasiWilayah ON MSPKMutasiWilayah.NoID=MSPKMutasiWilayahD.IDHeader WHERE MSPKMutasiWilayahD.QtyPcs-IsNull((SELECT SUM(MPackingMutasiWilayahD.QtyPcs) FROM MPackingMutasiWilayahD WHERE MPackingMutasiWilayahD.IDHeader<>" & NoID & " AND MPackingMutasiWilayahD.IDSPKMutasiWilayahD=MSPKMutasiWilayahD.NoID),0)>0 AND MSPKMutasiWilayah.IDWilayahDari=" & NullTolong(txtWilayahDiminta.EditValue) & " AND MSPKMutasiWilayah.IsPosted=1 AND (MSPKMutasiWilayah.IsSelesai=0 OR MSPKMutasiWilayah.IsSelesai IS NULL)" & vbCrLf & _
    ...
    Private Sub txtKodeReff_ButtonClick(ByVal sender As Object, ByVal e As DevExpress.XtraEditors.Controls.ButtonPressedEventArgs) Handles txtKodeReff.ButtonClick
        ...
        Select Case e.Button.Index
            Case 1
                InsertIntoDetilPacking(NullTolong(txtKodeReff.EditValue))
                RefreshDetil()
                txtKodeReff.Focus()
        End Select
    End Sub
    ...
    Private Sub InsertIntoDetilPacking(ByVal IDReq As Long)
        Dim SQL As String = ""
        Dim ds As New DataSet
        Try
            SQL = "SELECT MSPKMutasiWilayahD.*, (MSPKMutasiWilayahD.Qty*MSPKMutasiWilayahD.Konversi)-IsNull((SELECT SUM(A.Qty*A.Konversi) FROM MPackingMutasiWilayahD A WHERE A.IDSPKMutasiWilayahD=MSPKMutasiWilayahD.NoID),0) AS SisaQty "
            SQL &= " FROM MSPKMutasiWilayahD WHERE MSPKMutasiWilayahD.IDHeader=" & IDReq & " AND (MSPKMutasiWilayahD.Qty*MSPKMutasiWilayahD.Konversi)-IsNull((SELECT SUM(A.Qty*A.Konversi) FROM MPackingMutasiWilayahD A WHERE A.IDSPKMutasiWilayahD=MSPKMutasiWilayahD.NoID),0)>0 "
            ds = ExecuteDataset("MSPKMutasiWilayah", SQL)
            With ds.Tables("MSPKMutasiWilayah")
                If .Rows.Count >= 1 Then
                    For i As Integer = 0 To .Rows.Count - 1
                        SQL = "INSERT INTO MPackingMutasiWilayahD (NoID ,IDHeader ,IDSPKMutasiWilayahD ,IDRequestMutasiWilayahD, IDBarang ,IDSatuan ,Konversi ,Qty ,QtyPCS ,Ctn ,Keterangan) VALUES (" & _
                              NullTolong(GetNewID("MPackingMutasiWilayahD", "NoID")) & ", " & NoID & ", " & NullTolong(.Rows(i).Item("NoID")) & ", " & NullTolong(.Rows(i).Item("IDRequestMutasiWilayahD")) & ", " & NullTolong(.Rows(i).Item("IDBarang")) & ", " & NullTolong(.Rows(i).Item("IDSatuan")) & ", " & _
    ...
    Private Sub txtKodeReff_EditValueChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles txtKodeReff.EditValueChanged
        ...
        Try
            ...
            SQL &= " WHERE MSPKMutasiWilayah.NoID = " & NullTolong(txtKodeReff.EditValue)
            ds = ExecuteDataset("Tabel", SQL)
            If ds.Tables("Tabel").Rows.Count >= 1 Then
                txtWilayahDiminta.EditValue = NullTolong(ds.Tables(0).Rows(0).Item("IDWilayahDari"))
                txtWilayah.EditValue = NullTolong(ds.Tables(0).Rows(0).Item("IDWilayahUntuk"))
