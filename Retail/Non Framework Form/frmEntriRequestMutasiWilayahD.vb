...
Public Class frmEntriRequestMutasiWilayahD
    Public IsNew As Boolean = True
    Public IsPosted As Boolean = False
    Public NoID As Long = -1
    Public IDHeader As Long = -1
    Public IDWilayahDari As Long = -1
    Public FormPemanggil As frmEntriRequestMutasiWilayah
    ...
    Private Sub LoadData()
        Try
            SQL = "SELECT MRequestMutasiWilayahD.*, MBarang.CtnPcs, MBarang.Kode AS KodeBarang, MBarang.Nama AS NamaBarang, MSatuan.Nama AS NamaSatuan "
            ...
            SQL &= " WHERE MRequestMutasiWilayahD.NoID= " & NoID
            oDS = New DataSet()
    ...
    Private Function Simpan() As Boolean
        Try
            If IsNew Then
                SQL = "INSERT INTO [MRequestMutasiWilayahD] ([NoID],[IDHeader],[IDBarang],[IDSatuan],[Konversi],[Qty],[QtyPCS],[Ctn],[Keterangan]) VALUES ("
                SQL &= NullTolong(GetNewID("MRequestMutasiWilayahD", "NoID")) & ","
                SQL &= IDHeader & ","
                ...
                SQL &= ")"
            Else
                SQL = "UPDATE MRequestMutasiWilayahD SET "
    ...
    Private Function IsValidasi() As Boolean
        ...
        'Qty Melebihi Stock Wilayah
        If SisaStockWilayah < txtQtyPcs.EditValue Then
            If XtraMessageBox.Show("Nama stock " & txtNamaStock.Text & " melebihi stock wilayah " & NullTostr(EksekusiSQlSkalarNew("SELECT Nama FROM MWilayah WHERE NoID=" & IDWilayahDari)) & vbCrLf & "Yakin ingin melanjutkan penyimpanan?", NamaAplikasi, MessageBoxButtons.YesNo, MessageBoxIcon.Warning) = Windows.Forms.DialogResult.No Then
                txtQty.Focus()
                Return False
                Exit Function
            End If
    ...
    Private Sub frmEntriPOD_FormClosing(ByVal sender As Object, ByVal e As System.Windows.Forms.FormClosingEventArgs) Handles Me.FormClosing
        Try
            ...
            Else
                If IsFastEntri Then
                    EksekusiSQL("DELETE FROM MRequestMutasiWilayahD WHERE NoID=" & NoID)
                End If
                FormPemanggil.Show()
                FormPemanggil.Focus()
            End If
    ...
    Private Sub frmEntriPOD_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Try
            ...
            HighLightTxt()
            LayoutControlGroup3.Text = "Informasi Stock Wilayah " & NullTostr(EksekusiSQlSkalarNew("SELECT Nama FROM MWilayah WHERE NoID=" & IDWilayahDari))
        Catch ex As Exception
            XtraMessageBox.Show(ex.Message, NamaAplikasi, MessageBoxButtons.OK, MessageBoxIcon.Error)
        Finally
            Windows.Forms.Cursor.Current = Cursors.Default
    ...
    Private Sub frmEntriRequestMutasiWilayahD_Shown(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Shown
        Try
            txtBarang.Focus()
        Catch ex As Exception
