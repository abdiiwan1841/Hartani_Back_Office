...
Public Class frmEntriStockOpnameD
    ...
    Public JamSO As String = ""
    Public FormPemanggil As frmEntriStockOpname = Nothing
    ...
    Private Sub LoadData()
        Try
            SQL = "SELECT MStockOpnameD.*, MBarang.CtnPcs, MBarang.Kode AS KodeBarang, MBarang.Nama AS NamaBarang, MSatuan.Nama AS NamaSatuan "
            SQL &= " FROM (MStockOpnameD LEFT JOIN MBarang ON MBarang.NoID=MStockOpnameD.IDBarang) "
            SQL &= " LEFT JOIN MSatuan ON MSatuan.NoID=MStockOpnameD.IDSatuan "
            SQL &= " WHERE MStockOpnameD.NoID= " & NoID
            oDS = New DataSet()
            ...
            If oDS.Tables(0).Rows.Count >= 1 Then
                With oDS.Tables("MPOD").Rows(0)
                    ...
                    txtQtyKomputer.EditValue = NullToDbl(.Item("QtyKomputer"))
                    txtQtyFisik.EditValue = NullToDbl(.Item("QtyFisik"))
    ...
    Private Function Simpan() As Boolean
        Try
            If IsNew Then
                SQL = "INSERT INTO [MStockOpnameD] ([QtyKomputer],[QtyFisik],[NoID],[IDHeader],[IDBarang],[IDGudang],[IDSatuan],[Konversi],[Qty],[QtyPCS],[Keterangan],[HargaPokok],[Jumlah],[IDBarangD]) VALUES ("
                SQL &= FixKoma(txtQtyKomputer.EditValue) & ","
                SQL &= FixKoma(txtQtyFisik.EditValue) & ","
        ...
        End Try
    End Function
    Private Function AdaDiTransaksiLain(ByVal IDBarangD As Long) As Boolean
        Try
            SQL = "SELECT COUNT(MStockOpnameD.NoID) FROM MStockOpnameD INNER JOIN MStockOpname ON MStockOpname.NoID=MStockOpnameD.IDHeader WHERE MStockOpnameD.IDBarangD=" & IDBarangD & " AND MStockOpnameD.NoID<>" & NoID & " AND (MStockOpname.Tanggal>='" & FormPemanggil.txtTgl.DateTime.ToString("yyyy/MM/dd") & "' AND MStockOpname.Tanggal<'" & FormPemanggil.txtTgl.DateTime.AddDays(1).ToString("yyyy/MM/dd") & "')"
    ...
    Private Function IsValidasi() As Boolean
        ...
        If AdaDiTransaksiLain(NullToLong(txtBarcode.EditValue)) Then
            If XtraMessageBox.Show("Item ini sudah ada di transaksi lain. Cek lebih dahulu dan Qty tambahkan saja." & vbCrLf & "Jika ingin melanjutkan tekan Yes untuk merevisi ulang tekan No.", NamaAplikasi, MessageBoxButtons.YesNo, MessageBoxIcon.Warning, MessageBoxDefaultButton.Button2) = Windows.Forms.DialogResult.Yes Then
    ...
    Sub RefreshStock()
        ...
        Try
            ...
            dlg.Show()
            Windows.Forms.Cursor.Current = Cursors.WaitCursor
            If Not FormPemanggil Is Nothing Then
                If FormPemanggil.RadioGroup1.EditValue = 1 Then
                    SQL = "SELECT MWilayah.Nama AS Wilayah, MGudang.Nama AS Gudang, MBarang.Nama + ' ' + IsNull(MBarangD.Varian,'') AS NamaStock, MBarang.Kode AS KodeStock, SUM((MKartuStok.QtyMasuk*MKartuStok.Konversi)-(MKartuStok.QtyKeluar*MKartuStok.Konversi)) AS QtySisa " & _
                          " FROM MKartuStok LEFT JOIN MBarang ON MBarang.NoID=MKartuStok.IDBarang " & _
                          " LEFT JOIN MBarangD ON MBarangD.NoID=MKartuStok.IDBarangD " & _
                          ...
                          " AND MKartuStok.IDBarangD=" & NullToLong(txtBarcode.EditValue) & vbCrLf & _
    ...
    Private Sub HitungJumlah()
        Try
            ...
            txtJumlah.EditValue = txtQtyFisik.EditValue * txtKonversi.EditValue * txtHargaPokok.EditValue
        Catch ex As Exception
    ...
    Private Sub txtBarang_EditValueChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles txtBarang.EditValueChanged
        ...
        Try
            ...
            If Ds.Tables("Tabel").Rows.Count >= 1 Then
                ...
                If txtKonversi.EditValue = 0 Then
                    txtQtyKomputer.EditValue = NullToDbl(EksekusiSQlSkalarNew("SELECT SUM((MKartuStok.Qtymasuk*MKartuStok.Konversi)-(MKartuStok.QtyKeluar*MKartuStok.Konversi)) AS Qty FROM MKartuStok WHERE IDGudang=" & NullTolong(txtGudang.EditValue) & " AND IDBarang=" & NullTolong(txtBarang.EditValue) & " AND Tanggal<='" & JamSO & "'")) / 1
                Else
                    txtQtyKomputer.EditValue = NullToDbl(EksekusiSQlSkalarNew("SELECT SUM((MKartuStok.Qtymasuk*MKartuStok.Konversi)-(MKartuStok.QtyKeluar*MKartuStok.Konversi)) AS Qty FROM MKartuStok WHERE IDGudang=" & NullTolong(txtGudang.EditValue) & " AND IDBarang=" & NullTolong(txtBarang.EditValue) & " AND Tanggal<='" & JamSO & "'")) / NullToDbl(txtKonversi.EditValue)
                End If
    ...
    Private Sub RubahSatuan()
        ...
        Try
            ...
            If Ds.Tables("Tabel").Rows.Count >= 1 Then
                ...
                If txtKonversi.EditValue = 0 Then
                    txtQtyKomputer.EditValue = NullToDbl(EksekusiSQlSkalarNew("SELECT SUM((MKartuStok.Qtymasuk*MKartuStok.Konversi)-(MKartuStok.QtyKeluar*MKartuStok.Konversi)) AS Qty FROM MKartuStok WHERE IDGudang=" & NullTolong(txtGudang.EditValue) & " AND IDBarang=" & NullTolong(txtBarang.EditValue) & " ")) / 1
                Else
                    txtQtyKomputer.EditValue = NullToDbl(EksekusiSQlSkalarNew("SELECT SUM((MKartuStok.Qtymasuk*MKartuStok.Konversi)-(MKartuStok.QtyKeluar*MKartuStok.Konversi)) AS Qty FROM MKartuStok WHERE IDGudang=" & NullTolong(txtGudang.EditValue) & " AND IDBarang=" & NullTolong(txtBarang.EditValue) & " ")) / NullToDbl(txtKonversi.EditValue)
                End If
    ...
    Private Sub TextEdit1_EditValueChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles txtQtyFisik.EditValueChanged

    End Sub

    Private Sub TextEdit1_LostFocus(ByVal sender As Object, ByVal e As System.EventArgs) Handles txtQtyFisik.LostFocus
        txtQty.EditValue = (txtQtyFisik.EditValue - txtQtyKomputer.EditValue)
        HitungJumlah()
    End Sub

    Private Sub txtQtyKomputer_EditValueChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles txtQtyKomputer.EditValueChanged
        txtQty.EditValue = (txtQtyFisik.EditValue - txtQtyKomputer.EditValue)
        HitungJumlah()
    End Sub
